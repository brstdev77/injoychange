...
use backend\models\PilotLeadershipCategorySearch;
 ...
 */
class PilotLeadershipCategoryController extends Controller {
    ...
    public function actionIndex() {
        $searchModel = new PilotLeadershipCategorySearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $model = new PilotLeadershipCategory();
    ...
    public function actionDelete($id) {
        ...
        $model = $this->findModel($id);
        $model = PilotLeadershipCorner::find()->where(['category_id' => $id])->all();
        if ($status != 1 || $status != 2 || empty($status)) {
            foreach ($model as $val) {
                if (!empty($val->lightbox_image)) {
                    if (@file_get_contents('http://devinjoyglobal.com/pilot/backend/web/img/leadership-corner-images/lightbox-image/' . $val->lightbox_image)) {
                        unlink('img/leadership-corner-images/lightbox-image' . $val->lightbox_image);
                    }
                }
                if (!empty($val->dashboard_image)) {
                    if (@file_get_contents('http://devinjoyglobal.com/pilot/backend/web/img/leadership-corner-images/dashboard-image/' . $val->dashboard_image)) {
                        unlink('img/leadership-corner-images/dashboard-image' . $val->dashboard_image);
                    }
                }
            }
            $this->findModel($id)->delete();
            \Yii::$app->db->createCommand()->delete('pilot_leadership_corner', ['category_id' => $id])->execute();
    ...
    protected function findModel($id) {
        if (($model = PilotLeadershipCategory::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionLoadTag() {
        ...
        $model = new PilotLeadershipCategory;
        $model1 = new Pilottags;
        $modal = PilotLeadershipCategory::find()->where(['id' => $id])->one();
        if (!empty($modal)):
        ...
        if (Yii::$app->request->post()) {
            $id = Yii::$app->request->post()['PilotLeadershipCategory']['id'];
            $modal = PilotLeadershipCategory::find()->where(['id' => $id])->one();
            $tags = explode(',', Yii::$app->request->post()['PilotTags']['tags']);
            // $tags = array_filter($arr);
            foreach ($tags as $key => $value):
            ...
            endforeach;
            if ($modal->category_name != Yii::$app->request->post()['PilotLeadershipCategory']['category_name']):
                $modal->category_name = Yii::$app->request->post()['PilotLeadershipCategory']['category_name'];
            endif;
