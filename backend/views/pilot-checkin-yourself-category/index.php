...
$highlight_tag1 = Yii::$app->request->get('PilotCheckinYourselfCategorySearch');
if ($highlight_tag1 != '') {
...
$tags_name = [];
?>
<div class="pilot-checkin-yourself-category-index">
    ...
    <div class="grid_weekly_category grid_verical">
        <div class="box box-warning">
            <div class="box-header">
                <h3 class="box-title">Check In With Yourself</h3>
            ...
            <?php if (PilotInhouseUser::isUserAdmin(Yii::$app->user->identity->id)): ?>
                ...
                GridView::widget([
                    ...
                    'columns' => [
                        ...
                        [
                            'label' => 'Checkin Yourself',
                            'format' => 'raw',
                            'value' => function ($model) {
                                $countcorner = backend\models\PilotCheckinYourselfData::getRecords($model->id);
                                return '<i class="fa fa-paste"></i>&nbsp;&nbsp;' . $countcorner . ' Core Value&nbsp;&nbsp;<a href="ajaxcheckin?cid=' . $model->id . '" class="chickinValue btn btn-default"  data-target="#add_core_values" class="btn btn-default">Manage</a>';
                            },
                        ],
                        //'user_id',
                        //'created',
                        ...
                        ['class' => 'yii\grid\ActionColumn',
                            ...
                            'buttons' => [
                                ...
                                'delete' => function ($url, $model) {
                                    $checkin_yourself = array();
                                    $game_obj = PilotCreateGame::find()->all();
                                    foreach ($game_obj as $game):
                                        $checkin_yourself[] = $game->checkin_yourself_content;
                                    endforeach;
                                    if (!(in_array($model->id, $checkin_yourself))):
                                        return "<a data-method='post' data-confirm='Are you sure you want to delete this item?'class=' grid btn btn-default' href='" . Yii::$app->homeUrl . "pilot-checkin-yourself-category/delete?id=" . $model->id . "'><i class='fa fa-trash-o'></i>&nbsp;Delete</a>";
                                    else:
...
<div class="container">
    <div class="modal fade" id="add_core_values" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Add Check In With Yourself</h4>
                </div>
                <div class="modal-body">
