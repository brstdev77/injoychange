...
/* @var $searchModel backend\models\PilotTodayslessonCornerSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Pilot Today Lesson Corners';
$this->params['breadcrumbs'][] = ['label' => 'pilot-todaylesson-corner', 'url' => [Yii::$app->homeUrl.'pilot-todayslesson-corner/index?cid='.Yii::$app->request->get('cid')]];
$this->params['breadcrumbs'][] = $this->title;
$searchModel = new PilotTodayslessonCornerSearch();
//$dataProvider = $searchModel->search(Yii::$app->request->queryParams);
$dataProvider = new ActiveDataProvider([
    'query' => PilotTodayslessonCorner::find()->where(['category_id' => Yii::$app->request->get('cid')]),
        ]);
$category = PilotTodayslessonCategory::find()->where(['id' => Yii::$app->request->get('cid')])->one();
if (!$category) {

    return Yii::$app->getResponse()->redirect(Yii::$app->homeUrl . "pilot-todayslesson-category/index");
}
                ...
                [
                    ...
                    'value' => function ($model) {
                        ...
                        return '<div class="row"><div class="col-md-12" style="text-align:center"><b>' . $model->title . '</b></div><div class="col-md-4" style="text-align:center"><img style="width:100px;text-align:left;" src=" ' . $img_src . ' "></div>
                            <div class="col-md-8" style="word-wrap: break-word;word-break:break-all;">' . $model->description1 . '</div></div>';
                    },
                ],
//                [
...
//                ],
                ...
                [
                    ...
                    'value' => function ($model) {
                        ...
                        return backend\models\PilotTodayslessonCorner::getUpdated($model->id, $model->user_id);
                    },
                ],
                // 'user_id',
                ...
                ['class' => 'yii\grid\ActionColumn',
                    ...
                    'buttons' => [
                        'update' => function ($url, $model) {
                            return "<a question = '" . $model->question . "' answer_type = '" . $model->answer_type . "' leadership_id = '" . $model->id . "' week='" . $model->week . "' title='" . $model->title . "' dashboardimg='" . $model->dashboard_image . "' lightboximg='" . $model->popup_image . "' description='" . $model->description1 . " 'designation=' " . $model->designation . " 'class=' grid btn btn-default edit_leadership_corner' href='" . Yii::$app->homeUrl . "pilot-todayslesson-corner/update?id=" . $model->id . "&cid=" . Yii::$app->request->get('cid') . "'><i class='fa fa-pencil-square-o'></i> Edit</a>";
                        },
                        'delete' => function ($url, $model) {
                            return "<a data-method='post' data-confirm='Are you sure you want to delete this item?'class=' grid btn btn-default' href='" . Yii::$app->homeUrl . "pilot-todayslesson-corner/delete?id=" . $model->id . "'><i class='fa fa-trash-o'></i>&nbsp;Delete</a>";
                        },
                    ],
                ],
            ],
