 ...
 */
class PilotLeadershipCorner extends \yii\db\ActiveRecord {
    ...
    public static function tableName() {
        return 'pilot_leadership_corner';
    }
    ...
    public function rules() {
        return [
            [['title', 'week', 'question', 'file_link', 'popup_image', 'dashboard_image', 'answer_type'], 'required'],
...
//                'whenClient' => "function (attribute, value) { if($(!'#pilotleadershipcorner-answer_type input').is(':checked')){ return false; } ; }"],
    ...
    public static function getUpdated($id, $uid) {
        $models = PilotLeadershipCorner::find()->where(['id' => $id])->one();
    ...
    public function validateDescription($attribute, $params, $validator) {
        if (strlen($this->$attribute) > 2000) {
            $this->addError($attribute, 'Firstname must not contain more than 800 Characters');
        }
    }
    ...
    public static function getCountclient($id) {
        $models = Company::find()->where(['id' => $id])->one();
        $countmodel = PilotLeadershipCorner::find()->where(['client_listing' => $models->id])->all();
