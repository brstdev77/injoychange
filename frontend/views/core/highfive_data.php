...
if (count($all_highfivecomment) == 0):
    ...
    foreach ($all_highfivecomment as $hghfv):
        ...
        //Comment Image
        $comment_image = PilotFrontCoreHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $hghfv->id])->all();
        ?>
        <?php if ($j == 1): ?>
                ...
                <div class=count>
                    ...
                    if (!empty($comment_option)):
                        ...
                        <div class=comment_count>
                            <span> (<span class="c_count <?= $hghfv->id; ?>"><?= count($total_comments); ?></span>)<a class='modal-load' href="highfive-usercomment-modal?uid=<?= $user_id; ?>&cid=<?= $hghfv->id; ?>" data-modal-id="highfive-usercomment" onclick='saveusercomment(this, event)' style="color:hsl(25, 100%, 50%)"> Add Comment</a></span>
                        </div>
                    <?php endif; ?>
            ...
            <div id="hf_<?= $hghfv->id; ?>" class="High-5 <?= $row_cls; ?>" class="first_row" style="">
                ...
                <div class=count>
                    ...
                    if (!empty($comment_option)):
                        ...
                        ?>
                        <div class=comment_count>
                            <span> (<span class="c_count <?= $hghfv->id; ?>"><?= count($total_comments); ?></span>)<a class='modal-load' href="highfive-usercomment-modal?uid=<?= $user_id; ?>&cid=<?= $hghfv->id; ?>" data-modal-id="highfive-usercomment" style='color:hsl(25, 100%, 50%)' onclick='saveusercomment(this, event)'> Add Comment</a></span>
                        </div>
                    <?php endif; ?>
                    <input type="hidden" value="<?= $enable_comment; ?>" id="add_comment_feature"/>
                </div>
