...
$firstweek_highfive = PilotFrontCustomerHighfive::find()->where(['user_id' => Yii::$app->user->identity->id, 'feature_label' => highfiveComment, 'week_no' => 1])->count();
$firstweek_daily = PilotFrontCustomerDailyinspiration::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 1])->count();
$firstweek_question = PilotFrontCustomerQuestion::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 1])->count();
$firstweek_know = PilotFrontCustomerKnowcorner::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 1])->count();
$firstweek_audio = PilotFrontCustomerWeeklychallenge::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 1])->count();

$secondweek_highfive = PilotFrontCustomerHighfive::find()->where(['user_id' => Yii::$app->user->identity->id, 'feature_label' => highfiveComment, 'week_no' => 2])->count();
$secondweek_daily = PilotFrontCustomerDailyinspiration::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 2])->count();
$secondweek_question = PilotFrontCustomerQuestion::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 2])->count();
$secondweek_know = PilotFrontCustomerKnowcorner::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 2])->count();
$secondweek_audio = PilotFrontCustomerWeeklychallenge::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 2])->count();


$thirdweek_highfive = PilotFrontCustomerHighfive::find()->where(['user_id' => Yii::$app->user->identity->id, 'feature_label' => highfiveComment, 'week_no' => 3])->count();
$thirdweek_daily = PilotFrontCustomerDailyinspiration::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 3])->count();
$thirdweek_question = PilotFrontCustomerQuestion::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 3])->count();
$thirdweek_know = PilotFrontCustomerKnowcorner::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 3])->count();
$thirdweek_audio = PilotFrontCustomerWeeklychallenge::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 3])->count();


$fourthweek_highfive = PilotFrontCustomerHighfive::find()->where(['user_id' => Yii::$app->user->identity->id, 'feature_label' => highfiveComment, 'week_no' => 4])->count();
$fourthweek_daily = PilotFrontCustomerDailyinspiration::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 4])->count();
$fourthweek_question = PilotFrontCustomerQuestion::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 4])->count();
$fourthweek_know = PilotFrontCustomerKnowcorner::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 4])->count();
$fourthweek_audio = PilotFrontCustomerWeeklychallenge::find()->where(['user_id' => Yii::$app->user->identity->id, 'week_no' => 4])->count();
if ($firstweek_highfive >= 15 && $firstweek_daily >= 5 && $firstweek_question >= 1 && $firstweek_know >= 2 && $firstweek_audio >= 3):
    $first = $baseurl . '/images/1_customer.png';
else:
    $first = $baseurl . '/images/star1_empty.png';
endif;
if ($secondweek_highfive >= 15 && $secondweek_daily >= 5 && $secondweek_question >= 1 && $secondweek_know >= 2 && $secondweek_audio >= 3):
    $second = $baseurl . '/images/2_customer.png';
else:
    $second = $baseurl . '/images/star2_empty.png';
endif;
if ($thirdweek_highfive >= 15 && $thirdweek_daily >= 5 && $thirdweek_question >= 1 && $thirdweek_know >= 2 && $thirdweek_audio >= 3):
    $third = $baseurl . '/images/3_customer.png';
else:
    $third = $baseurl . '/images/star3_empty.png';
endif;
if ($fourthweek_highfive >= 15 && $fourthweek_daily >= 5 && $fourthweek_question >= 1 && $fourthweek_know >= 2 && $fourthweek_audio >= 3):
    $fourth = $baseurl . '/images/4_customer.png';
else:
    $fourth = $baseurl . '/images/star4_empty.png';
endif;
$new_notifs = PilotFrontCustomerNotifications::find()->where(['user_id' => Yii::$app->user->identity->id, 'notif_status' => 1])->all();
                    ...
                    <div class=col-md-12>
                        ...
                        <nav class="navbar navbar-inverse">
                            ...
                            <div id=collapse class="collapse navbar-collapse navbar-left">
                                <ul class="nav navbar-nav">
                                    <li><a href="<?= $baseurl; ?>/customer/how-it-work">how it works</a></li>
                                    <!--<li><a href="#">Additional Resources</a></li>-->
                                    <li><a href="<?= $baseurl; ?>/customer/leaderboard">Leaderboard</a></li>
                                    <li><a href="<?= $baseurl; ?>/customer/tech-support">Tech Support</a></li>
                        ...
                        <?php if (!empty($notifs_bell)): ?>
                            <div class="notification bell">
                                <a href='javascript:void(0)'><img class=notification src="<?= $baseurl; ?>/images/notification-customer.png" alt=notification height=32 width=32 /></a>
                            </div>
