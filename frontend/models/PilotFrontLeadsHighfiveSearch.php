 ...
 */
class PilotFrontLeadsHighfiveSearch extends PilotFrontLeadsHighfive {
    ...
    public function search($params) {
        if ($params['shouttype'] == 1):
            ...
            if (in_array(11, $feature_array)):
                $subQuery = PilotFrontLeadsHighfive::find()->where(['feature_label' => 'highfiveCommentImage'])->select('linked_feature_id');
                $query = PilotFrontLeadsHighfive::find()
                        ->where(['not in', 'pilot_front_leads_highfive.id', $subQuery])
                        ->andWhere(['game_id' => $game, 'pilot_front_leads_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_leads_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            else:
                $query = PilotFrontLeadsHighfive::find()
                        ->where(['game_id' => $game, 'pilot_front_leads_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_leads_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            endif; 
            ...
            $user = PilotFrontUser::find()->where(['username' => $username, 'challenge_id' => $game, 'company_id' => $comp_id])->one();
...
//    }
    ...
    public function searchbackend($params, $id) {
        ...
        $query = $model_highfive::find()->where(['game_id' => $challid, 'pilot_front_' . $challenge_obj->challenge_abbr_name . '_highfive.company_id' => $cid, 'pilot_front_' . $challenge_obj->challenge_abbr_name . '_highfive.challenge_id' => $id])
