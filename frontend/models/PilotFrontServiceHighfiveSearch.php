  ...
  public function search($params) {
        if ($params['shouttype'] == 1):
            ...
            if (in_array(11, $feature_array)):
                $subQuery = PilotFrontServiceHighfive::find()->where(['feature_label' => 'highfiveCommentImage'])->select('linked_feature_id');
                $query = PilotFrontServiceHighfive::find()
                        ->where(['not in', 'pilot_front_service_highfive.id', $subQuery])
                        ->andWhere(['game_id' => $game, 'pilot_front_service_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_service_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            else:
                $query = PilotFrontServiceHighfive::find()
                        ->where(['game_id' => $game, 'pilot_front_service_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_service_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            endif;
            ...
            $username = $params['PilotFrontServiceHighfiveSearch']['user_id'];
