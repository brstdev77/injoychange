...
/**
 * PilotFrontCustomerHighfiveSearch represents the model behind the search form about `frontend\models\PilotFrontCustomerHighfive`.
 */
class PilotFrontCustomerHighfiveSearch extends PilotFrontCustomerHighfive {
...
//            return $dataProvider;
            ...
            $comp_id = Yii::$app->user->identity->company_id;
            $query = PilotFrontCustomerHighfive::find()
                    ->where(['game_id' => $game, 'pilot_front_customer_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment',])
                    ->joinWith('userinfo')
                    ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            $dataProvider = new ActiveDataProvider([
                'query' => $query,
            ...
            $username = $params['PilotFrontCustomerHighfiveSearch']['user_id'];
            $user = PilotFrontUser::find()->where(['username' => $username])->one();
            $this->load($params);
//    if (!$this->validate()) {
//      return $dataProvider;
