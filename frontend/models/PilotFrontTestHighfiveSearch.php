...
/**
 * PilotFrontTestHighfiveSearch represents the model behind the search form about `frontend\models\PilotFrontTestHighfive`.
 */
class PilotFrontTestHighfiveSearch extends PilotFrontTestHighfive {
...
//            return $dataProvider;
            ...
            $comp_id = Yii::$app->user->identity->company_id;
            $query = PilotFrontTestHighfive::find()
                    ->where(['game_id' => $game, 'pilot_front_test_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment',])
                    ->joinWith('userinfo')
                    ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            $dataProvider = new ActiveDataProvider([
                'query' => $query,
            ...
            $username = $params['PilotFrontTestHighfiveSearch']['user_id'];
            $user = PilotFrontUser::find()->where(['username' => $username,'challenge_id' => $game])->one();
            $this->load($params);
//    if (!$this->validate()) {
//      return $dataProvider;
//    }
