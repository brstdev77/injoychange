...
/**
 * PilotFrontPeakHighfiveSearch represents the model behind the search form about `frontend\models\PilotFrontPeakHighfive`.
 */
class PilotFrontPeakHighfiveSearch extends PilotFrontPeakHighfive {
    ...
    public function search($params) {
        ...
        if ($params['shouttype'] == 1):
            ...
            if (in_array(11, $feature_array)):
                $subQuery = PilotFrontPeakHighfive::find()->where(['feature_label' => 'highfiveCommentImage'])->select('linked_feature_id');
                $query = PilotFrontPeakHighfive::find()
                        ->where(['not in', 'pilot_front_peak_highfive.id', $subQuery])
                        ->andWhere(['game_id' => $game, 'pilot_front_peak_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_peak_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            else:
                $query = PilotFrontPeakHighfive::find()
                        ->where(['game_id' => $game, 'pilot_front_peak_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'pilot_front_peak_highfive.challenge_id' => $game_id])
                        ->joinWith('userinfo')
                        ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            ...
            $username = $params['PilotFrontPeakHighfiveSearch']['user_id'];
