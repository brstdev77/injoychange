 ...
 */
class PilotFrontLeadsStepsApiData extends \yii\db\ActiveRecord {

  /**
   * @inheritdoc
   */
  public static function tableName() {
    return 'pilot_front_leads_steps_api_data';
  }
  ...
  public function attributeLabels() {
   ...
   public static function getUserSteps($user_id) {
    $dayset = date('d/m/Y');
    $get_rec = PilotFrontLeadsStepsApiData::find()->where(['user_id' => $user_id, 'date' => $dayset])->one();
    $total_steps = 0;
    if (!empty($get_rec)):
   ...
   public static function getUserStepsforchallenge($user_id) {
    $total_steps = PilotFrontLeadsStepsApiData::find()->where(['user_id' => $user_id])->sum('steps');
   ...
   public static function getAllUsersStepsforchallenge() {
    ...
    $users = PilotFrontLeadsStepsApiData::find()->select(['user_id'])->where(['company_id' => $company_id, 'team_id' => $team_id])->groupBy(['user_id'])->All();
    $users_steps_arr = [];
    foreach ($users as $user):
      $uid = $user->user_id;
      $user_total_steps = PilotFrontLeadsStepsApiData::find()->where(['user_id' => $uid])->sum('steps');
