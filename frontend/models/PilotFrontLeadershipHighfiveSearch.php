...
/**
 * PilotFrontLeadershipHighfiveSearch represents the model behind the search form about `frontend\models\PilotFrontLeadershipHighfive`.
 */
class PilotFrontLeadershipHighfiveSearch extends PilotFrontLeadershipHighfive {
...
//            return $dataProvider;
            ...
            $comp_id = Yii::$app->user->identity->company_id;
            $query = PilotFrontLeadershipHighfive::find()
                    ->where(['game_id' => $game, 'pilot_front_leadership_highfive.company_id' => $comp_id, 'feature_label' => 'highfiveComment',])
                    ->joinWith('userinfo')
                    ->orderBy(['dayset' => SORT_DESC, 'id' => SORT_DESC]);
            $dataProvider = new ActiveDataProvider([
                'query' => $query,
            ...
            $username = $params['PilotFrontLeadershipHighfiveSearch']['user_id'];
            $user = PilotFrontUser::find()->where(['username' => $username])->one();
            $this->load($params);
//    if (!$this->validate()) {
//      return $dataProvider;
