...
use frontend\models\PilotFrontServiceDailyinspiration;
use frontend\models\PilotFrontServiceLeadershipcorner;
use frontend\models\PilotFrontServiceWeeklychallenge;
use frontend\models\PilotFrontServiceShareawin;
use frontend\models\PilotFrontServiceHighfive;
use frontend\models\PilotFrontServiceHighfiveSearch;
use frontend\models\PilotFrontServiceCheckin;
use frontend\models\PilotFrontUser;
use frontend\models\PilotFrontServiceNotifications;
use frontend\models\PilotFrontServiceTotalPoints;
use frontend\models\PilotFrontServiceSurveyData;
use frontend\models\PilotFrontServiceStepsApiData;
use frontend\models\PilotFrontServiceStepsApiDataSearch;
use frontend\models\PilotFrontServiceStepsModal;
use frontend\models\PilotFrontServiceTechSupport;
use frontend\models\PilotFrontServiceRating;
...
use frontend\models\PilotFrontServiceEmail;
...
class ServiceController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '2' && $challenge_id != 0) {
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                if (in_array($team_id, $challenge_teams)):
                    ...
                    return $this->redirect(Url::to(['/service/welcome']));
                else:
                    return $this->redirect(Url::to(['/service/thankyou']));
                    return $this->goBack();
                endif;
            else:
                return $this->redirect(Url::to(['/service/thankyou']));
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontServiceDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            //Core Values
            ...
            $corevalues_entry = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => 'core_values_popup', 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $prev_today_values_currentday = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
            ...
            $total_weeks_entries_points = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum(points);
            ...
            $today_valuesModel = new PilotFrontServiceCheckin;
            if (Yii::$app->request->post('PilotFrontServiceCheckin')):
                $label = Yii::$app->request->post('PilotFrontServiceCheckin')['label'];
                $comment = Yii::$app->request->post('PilotFrontServiceCheckin')['comment'];
                $serial = Yii::$app->request->post('PilotFrontServiceCheckin')['serial'];
                $checkModel = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'serial' => $serial, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontServiceCheckin;
                    $model->game_id = $game;
            ...
            if (in_array(11, $feature_array)):
                $subQuery = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query = PilotFrontServiceHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(10);
                $all_highfives = $query->all();
                $sub_Query = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $Query = PilotFrontServiceHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                $sum_week_points = 0;
                $sub_Query1 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'week_no' => $week_no, 'challenge_id' => $game_id])->select('linked_feature_id');
                $Query1 = PilotFrontServiceHighfive::find()->where(['not in', 'id', $sub_Query1])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'week_no' => $week_no])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                //Makeup Days entry  
                $subQuery5 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query5 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery5])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id]);
                ...
                $subQuery2 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query2 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery2])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                //Makeup Days entry            
                $subQuery4 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query4 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery4])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id]);
            ...
            //end here
                ...
                //High Five 
                $all_highfives = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(40)->all();
                $prev_highfives_currentday = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset, 'challenge_id' => $game_id])->orderBy(['created' => SORT_ASC])->all();
                $hf_points = 10;
                $count_hf = 0;
                if (!empty($prev_highfives_currentday)):
                    $count_hf = count($prev_highfives_currentday);
            ...
            $highfiveModel = new PilotFrontServiceHighfive;
            if (Yii::$app->request->post('PilotFrontServiceHighfive')):
                $feature_label = Yii::$app->request->post('PilotFrontServiceHighfive')['feature_label'];
                $feature_value = Yii::$app->request->post('PilotFrontServiceHighfive')['feature_value'];
                $feature_serial = Yii::$app->request->post('PilotFrontServiceHighfive')['feature_serial'];
                $linked_feature_id = Yii::$app->request->post('PilotFrontServiceHighfive')['linked_feature_id'];
                $checkModel = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontServiceHighfive;
                    $model->game_id = $game;
                    $model->challenge_id = $game_id;
                    $model->user_id = $user_id;
                    $model->company_id = $comp_id;
                    ...
                    if (Yii::$app->request->post('hf_image_status')):
                        if (isset($_SESSION['hf_cmnt_img'])):
                            $image_model = new PilotFrontServiceHighfive;
...
//              die;
                            ...
                            if ($tagged_uid != $user_id):
                                $notif_model = new PilotFrontServiceNotifications;
            ...
            //Leadership Corner
            $leadership_entry_weekly = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $leadership_entry_currentday = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $leadership_entry_first = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $leadership_entry_sec = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();
            $leadership_entry_third = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third', 'challenge_id' => $game_id])->one();

            //Weekly Challenge
            $weekly_entry = PilotFrontServiceWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->one();
            ...
            //Share A Win 
            $shareawin_entry = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->all();
            //$shareawins_all = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])->joinWith('userinfo')->orderBy(['created' => SORT_DESC])->all();

            $shareawins_all = PilotFrontServiceShareawin::find()
                            ->where(['game_id' => $game, 'pilot_front_service_shareawin.company_id' => $comp_id, 'pilot_front_service_shareawin.challenge_id' => $game_id])
                            ->joinWith('userinfo')
            ...
            if (!empty($team_id)):
                $users_points_obj = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            else:
                $users_points_obj = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            endif;
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontServiceSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontServiceSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontServiceSurveyData')):
                $skipped_entry = PilotFrontServiceSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontServiceSurveyData');
                ...
                foreach ($survey_posted_questions as $key => $q_id):
                    $surveyModel = new PilotFrontServiceSurveyData();
            ...
            $fitness_data = PilotFrontServiceStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();

            //Congratulation Modal Show on 10K Steps reach per day..
            $show_10k_congrats = 'false';
            $steps_obj = PilotFrontServiceStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($steps_obj)):
                $total_stepsofday = $steps_obj->steps;
                if ($total_stepsofday >= 10000):
                    $check_already_readforday = PilotFrontServiceStepsModal::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'modal_read' => 'Yes'])->one();
                    if (empty($check_already_readforday)):
                        $show_10k_congrats = 'true';
                    endif;
                endif;
            endif;
            $ratingmodel = new PilotFrontServiceRating;
    ...
    public function actionCheckin() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        $shareawins_all = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
    ...
    public function actionHighFive() {
        ...
        $searchModel = new PilotFrontServiceHighfiveSearch();
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            //All Users Points - Company & Team Wise
            if (!empty($team_id)):
                $points_obj = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'challenge_id' => $game_id])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(10)
                    ->all();
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        else:
            $login_user_team_core_values = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 10;
        $week_entries = PilotFrontServiceDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
        ...
        //Makeup Days entry
        $total_points_dailyInsp = PilotFrontServiceDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->sum('points');
        ...
        $daily_model = new PilotFrontServiceDailyinspiration;
        $daily_email = new PilotFrontServiceEmail;
        $daily_entry = PilotFrontServiceDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontServiceDailyinspiration')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontServiceDailyinspiration')['dayset'];
            $checkModel = PilotFrontServiceDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            if (empty($checkModel)):
                $model = new PilotFrontServiceDailyinspiration;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionCoreValuesModal() {
        ...
        $count_week_entr = 0;
        $points = 5;
        $week_entries = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
        $count_week_entr = count($week_entries);
        if ($count_week_entr >= 5):
            $points = 0;
        endif;
        ...
        //Makeup Days entry
        $total_points_corevalue = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'challenge_id' => $game_id])
                ->sum('points');
        ...
        $corevalues_model = new PilotFrontServiceCheckin;
        $corevalues_entry = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontServiceCheckin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontServiceCheckin')['dayset'];
            $checkModel = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            if (empty($checkModel)):
                $model = new PilotFrontServiceCheckin;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
    ...
    public function actionLeadershipModal() {
        ...
        $leadership_model = new PilotFrontServiceLeadershipcorner;
        $leadership_entry = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontServiceLeadershipcorner')['user_id'];
            $tip_pos = Yii::$app->request->post('PilotFrontServiceLeadershipcorner')['tip_pos'];
            $dayset = Yii::$app->request->post('PilotFrontServiceLeadershipcorner')['dayset'];
            $checkModel = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontServiceLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontServiceLeadershipcorner;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionWeeklyModal() {
        ...
        if (empty($weekly_entry)):
            $weekly_model = new PilotFrontServiceWeeklychallenge;
        else:
            $weekly_model = $weekly_entry;
        endif;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontServiceWeeklychallenge')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontServiceWeeklychallenge')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontServiceWeeklychallenge')['comment'];
            $checkModel = PilotFrontServiceWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
            $total_points_weeklyvideo = PilotFrontServiceWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontServiceWeeklychallenge;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionShareABinModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $prev_entry_currentday = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])
        ...
        //Game Points 5 Days Per Week
        $sum_week_points = 0;
        $week_entries_points = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry       
        $total_points_shareawin = PilotFrontServiceShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->sum('points');
        ...
        //end here
        $shareawin_model = new PilotFrontServiceShareawin;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontServiceShareawin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontServiceShareawin')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontServiceShareawin')['comment'];
            $model = new PilotFrontServiceShareawin;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id, 'challenge_id' => $game_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontServiceHighfive;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            ...
            $model->save(false);
            //Save Notification for User in pilot_front_notifications Table
            $linked_feature_user_model = PilotFrontServiceHighfive::find()->where(['id' => $linked_feature_id])->one();
        ...
        $total_likes = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontServiceHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontServiceHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontServiceHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontServiceHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id, 'challenge_id' => $game_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontServiceHighfive;
            $model->challenge_id = $game_id;
        ...
        $allComments = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontServiceNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontServiceNotifications::find()->where(['id' => $notif_id])->one();
    ...
    public function actionGetUserPoints() {
        ...
        $total_core_values = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                        ->andWhere(['!=', 'label', 'core_values_popup'])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                        ->orderBy(['created' => SORT_DESC])->all();
        ...
        $total_points = PilotFrontUser::getUserPoints($game_obj);
        ...
        $total_actions = PilotFrontUser::getUserActions($game_obj);
        //Save the User Points to the Main Table of All Users Points of Service Challenge
        $check_entry = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->one();
        if (empty($check_entry)):
            $model = new PilotFrontServiceTotalPoints;
            $model->game_id = $game;
            $model->challenge_id = $game_id;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionGetCompanyUsersPoints() {
        ...
        $team_core_values_count = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_core_values');

        $total_actions_count = PilotFrontServiceTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_game_actions');
...
//                if ($month3 != 0):
//                    for ($d3 = 1; $d3 <= 31; $d3++) {
//                        $time3 = mktime(12, 0, 0, $month3, $d3, $year2);
//                        if (date('m', $time3) == $month3)
//                            $list_middle_dates[] = date('Y-m-d', $time3);
//                    }
//                endif;
//            endif;
//        endif;
...
//        }
//        if ($month2 < $month1):
//            $list_dates1 = array_merge($list_start_dates, $list_middle_dates);
//            $list_dates = array_merge($list_dates1, $list_end_dates);
//        else:
...
//        endif;
        ...
        foreach ($list_dates as $index => $val):
            ...
            foreach ($pointsArr as $date => $value):
                if (in_array($date, $gameDates)):
                    ...
                    $check_in_points_res = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $date, 'challenge_id' => $game_id])
                            ->andWhere(['!=', 'label', 'core_values_popup'])
                            ->sum('points');
        ...
        if (Yii::$app->request->post('PilotFrontServiceCheckin')):
            ...
            //Game Points 5 Days Per Week
            $sum_week_points = 0;
            $week_entries_points = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_asper_date, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
    ...
    public function actionGetCalRecord() {
        if (Yii::$app->request->isAjax == true) {
            ...
            $valuesLabel = ArrayHelper::map($valuesLabel_obj, 'core_values_name', 'core_values_name');
            $today_valuesModel = new PilotFrontServiceCheckin;
            $prev_values_currentday = PilotFrontServiceCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontServiceSurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionFitnessSteps() {
        ...
        foreach ($gamedates_array as $key => $gamedate):
            ...
            $fetch_data = PilotFrontServiceStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($fetch_data)):
    ...
    public function actionPostApiData() {
        ...
        if (isset($_REQUEST['uid']) && isset($_REQUEST['date']) && isset($_REQUEST['steps']) && isset($_REQUEST['calories']) && isset($_REQUEST['sleephr']) && isset($_REQUEST['distance'])):
            ...
            if (!empty($result_uid) && ($ruid > 0)):
                $getdata = PilotFrontServiceStepsApiData::find()->where(['user_id' => $uid, 'date' => $date])->one();
                //Get the Challenge(Game) ID
                $game = PilotFrontUser::getGameID('service');
                //User Company ID
                ...
                $week_asper_date = PilotFrontUser::getGameWeekAsPerDate($game_obj, $dayset);
                if (empty($getdata)):
                    $api_model = new PilotFrontServiceStepsApiData();
    ...
    public function actionTotalStepsModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $totalsteps_model = new PilotFrontServiceStepsApiData();
        //My Total Steps for Complete Challenge
        $user_total_steps = PilotFrontServiceStepsApiData::getUserStepsforchallenge($user_id);
        //Top five Users Total Steps for Complete Challenge
        $all_users_total_steps = PilotFrontServiceStepsApiData::getAllUsersStepsforchallenge();
    ...
    public function actionReadCongrats() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $congratsModel = new PilotFrontServiceStepsModal();
    ...
    public function actionTechSupport() {
        ...
        //Active Challenge Object (Start Date & End Date) 
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $model = new PilotFrontServiceTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontServiceTechSupport'];
            ...
            $newModel = new PilotFrontServiceTechSupport();
    ...
    public function actionUserlist() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $challenge_id = Yii::$app->user->identity->challenge_id;
        $user_idlist = PilotFrontServiceHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'game_id' => $challenge_id]);
...
//        }
    ...
    public function actionSeeAll() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontServiceNotifications::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])->orderBy(['created' => SORT_DESC]);
        $dataProvider = new ActiveDataProvider([
            'query' => $shareawins_all,
            'pagination' => [
                'pageSize' => 50,
    ...
    public function actionDailyEmail() {
        ...
        if (Yii::$app->request->post()):
            ...
            $send_email = Yii::$app
                    ->mailer
                    ->compose('send-image', ['model' => Yii::$app->request->post()['PilotFrontServiceEmail']['message'], 'daily_image_path' => $daily_image_path])
                    ->setFrom('support@injoyglobal.com')
                    ->setTo(Yii::$app->request->post()['PilotFrontServiceEmail']['email_id'])
                    ->setSubject('Daily Inspiration Image')
                    ->send();
//            $message = Yii::$app->mailer->compose('send-image', ['model' => Yii::$app->request->post()['PilotFrontServiceEmail']['message'], 'daily_image_path' => $daily_image_path])
//                    ->setFrom('support@injoyglobal.com')
//                    ->setTo(Yii::$app->request->post()['PilotFrontServiceEmail']['email_id'])
//                    ->setSubject('Daily Inspiration Image');
//            $message->send();
//            $to = Yii::$app->request->post()['PilotFrontServiceEmail']['email_id'];
//            $from = 'brstdev35@gmail.com';
//            $subject = 'Daily Inspiration Image';
...
//            $message .= '<html><body>';
//            $message .= '<img src="' . Yii::getAlias('@back_end') . '/img/daily-inspiration-images/' . $daily_image_path . '" alt="Website Change Request" />';
//            $message .= "</body></html>";
//
//            if (mail($to, $subject, $message, $headers)) {
//                $daily_email->email_id = Yii::$app->request->post()['PilotFrontServiceEmail']['email_id'];
...
//            }
    ...
    public function actionSaveRating() {
        ...
        $html = '';
        if (Yii::$app->request->post()):
            $model = PilotFrontServiceRating::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'company_id' => $comp_id])->one();
            if (empty($model)):
                $model = new PilotFrontServiceRating;
                $model->challenge_id = $game_obj->id;
                $model->game_id = $game;
                $model->company_id = $comp_id;
                $model->user_id = $user_id;
                $model->value = Yii::$app->request->post()['PilotFrontServiceRating']['value'];
                $model->dayset = $dayset;
                $model->week_no = $week_no;
                $model->created = time();
                $model->updated = time();
    ...
    public function actionHighFiveimages() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        $subQuery1 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query1 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(5);
        $all_highfiveImage = $query1->all();
        $query2 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
    ...
    public function actionLoadHighfiveImages() {
        ...
        if (count($all_highfiveImage) == 0):
            ...
            foreach ($all_highfiveImage as $hghfv1):
                ...
                $check_comment_liked = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->one();
                ...
                $total_likes = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->all();
                $comment_image = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->one();
    ...
    public function actionMoreHfComments() {
        ...
        if (in_array(11, $feature_array)):
            $subQuery = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $query = PilotFrontServiceHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count);
            $all_highfivecomment = $query->all();
            $fetched_comment = count($all_highfivecomment);
            $query1 = PilotFrontServiceHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
            $total_comment = $query1->count();
            $image = 0;
        //end here
        else:
            //High Five 
            $all_highfivecomment = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count)->all();
            $fetched_comment = count($all_highfivecomment);
            $query1 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->count();
            $total_comment = $query1->count();
            $image = 1;
//end here
        endif;
...
//        $total_comment = $query1->count();
    ...
    public function actionLoadmoreHighfiveImages() {
        ...
        $user_id = Yii::$app->user->identity->id;
        $subQuery1 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query1 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(5)->offset($offset);
        $all_highfiveImage = $query1->all();
        $query2 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $total_comment = $query2->count();
        $count1 = $offset + 5;
        $query3 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count1);
    ...
    public function actionSaveHighfiveComment() {
        ...
        $user_id = Yii::$app->user->identity->id;
        $team_id = Yii::$app->user->identity->team_id;
        $prev_highfives_currentday = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
        $hf_points = 10;
        $count_hf = 0;
        if (!empty($prev_highfives_currentday)):
            $count_hf = count($prev_highfives_currentday);
        ...
        //Game Points 5 Days Per Week - Shout Out Section
        $sum_week_points = 0;
        $week_entries_points = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry           
        $total_points_highfive = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->sum('points');
        if (!empty($total_points_highfive) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
            $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
            //echo $total_gameday;
        ...
        /* $checkModel = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'dayset' => $dayset])->one();
          if (empty($checkModel)): */
        $model = new PilotFrontServiceHighfive;
        $model->game_id = $game;
        $model->challenge_id = $game_id;
...
//              die;
    ...
    public function actionSaveHighfiveImageComment() {
        ...
        $sub_Query = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $Query = PilotFrontServiceHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $prev_highfives_currentday = $Query->all();
        $hf_points = 10;
        $count_hf = 0;
        if (!empty($prev_highfives_currentday)):
        ...
        //Game points for 5s in action
        $subQuery2 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query2 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery2])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $all_highfiveImage1 = $query2->all();
        if (empty($all_highfiveImage1)):
            $pointsimage = 30;
        else:
        ...
        $model->linked_feature_id = $linked_feature_id;
//                    if (Yii::$app->request->post('hf_image_status')):
//                        $type = PilotFrontServiceHighfiveType::find()->where(['id' => 2])->one();
//                        $type->type = 1;
//                        $type->created = time();
//                        $type->save(false);
...
//              die;
        ...
        if ($i >= 2 && isset($_SESSION['hf_cmnt_img'])):
            ...
            $output['serial'] = $feature_serial;
            $subQuery1 = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $query1 = PilotFrontServiceHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['challenge_id' => $game_id])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        ...
        elseif ($i <= 2):
            ...
            $output['serial'] = $feature_serial;
            $sub_Query = PilotFrontServiceHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $Query = PilotFrontServiceHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
            $prev_highfives_currentday = $Query->all();
            $count_hf = 0;
            if (!empty($prev_highfives_currentday)):
                $count_hf = count($prev_highfives_currentday);
    ...
    public function actionWeekly() {
        ...
        //Leadership Corner
        $leadership_entry_weekly = PilotFrontServiceWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
