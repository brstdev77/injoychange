...
use frontend\models\PilotFrontIntentionalDailyinspiration;
use frontend\models\PilotFrontIntentionalLeadershipcorner;
use frontend\models\PilotFrontIntentionalWeeklychallenge;
use frontend\models\PilotFrontIntentionalShareawin;
use frontend\models\PilotFrontIntentionalHighfive;
use frontend\models\PilotFrontIntentionalHighfiveSearch;
use frontend\models\PilotFrontIntentionalCheckin;
use frontend\models\PilotFrontUser;
use frontend\models\PilotFrontIntentionalNotifications;
use frontend\models\PilotFrontIntentionalTotalPoints;
use frontend\models\PilotFrontIntentionalSurveyData;
use frontend\models\PilotFrontIntentionalTechSupport;
...
use frontend\models\PilotFrontIntentionalEmail;
use kartik\rating\StarRating;

/**
 * IntentionalController implements the CRUD actions for Intentional model.
 */
class IntentionalController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '10' && $challenge_id != 0) {
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                if (in_array($team_id, $challenge_teams)):
                    ...
                    return $this->redirect(Url::to(['/intentional/welcome']));
                else:
                    return $this->redirect(Url::to(['/intentional/thankyou']));
                    return $this->goBack();
                endif;
            else:
                return $this->redirect(Url::to(['/intentional/thankyou']));
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontIntentionalDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            //Core Values
            ...
            $corevalues_entry = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => 'core_values_popup', 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $prev_today_values_currentday = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
            ...
            $total_weeks_entries_points = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum(points);
            ...
            $today_valuesModel = new PilotFrontIntentionalCheckin;
            if (Yii::$app->request->post()):
                $label = $_POST['label'];
                $comment = $_POST['comment'];
                $serial = $_POST['serial'];
                $checkModel = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'serial' => $serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontIntentionalCheckin;
                    $model->game_id = $game;
            ...
            if (in_array(11, $feature_array)):
                ...
                $query = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(20);
                $all_highfives = $query->all();
                $sub_Query = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $Query = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                $sum_week_points = 0;
                $sub_Query1 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'week_no' => $week_no, 'challenge_id' => $game_id])->select('linked_feature_id');
                $Query1 = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $sub_Query1])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'week_no' => $week_no])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                //Makeup Days entry  
                $subQuery5 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query5 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery5])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id]);
                ...
                $subQuery2 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query2 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery2])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
                ...
                //Makeup Days entry            
                $subQuery4 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
                $query4 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery4])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id, 'challenge_id' => $game_id]);
            ...
            //end here
                ...
                //High Five 
                $all_highfives = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(40)->all();
                $prev_highfives_currentday = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset, 'challenge_id' => $game_id])->orderBy(['created' => SORT_ASC])->all();
                $hf_points = 10;
                $count_hf = 0;
                if (!empty($prev_highfives_currentday)):
                    $count_hf = count($prev_highfives_currentday);
                ...
                //Game Points 5 Days Per Week - Shout Out Section
                $sum_week_points = 0;
                $week_entries_points = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
                        ->sum('points');
                if (!empty($week_entries_points)):
                    $sum_week_points = $week_entries_points;
                endif;
                ...
                //Makeup Days entry           
                $total_points_highfive = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                        ->sum('points');
                if (!empty($total_points_highfive) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
                    $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
                    //echo $total_gameday;
            ...
            $highfiveModel = new PilotFrontIntentionalHighfive;
            if (Yii::$app->request->post('PilotFrontIntentionalHighfive')):
                $feature_label = Yii::$app->request->post('PilotFrontIntentionalHighfive')['feature_label'];
                $feature_value = Yii::$app->request->post('PilotFrontIntentionalHighfive')['feature_value'];
                $feature_serial = Yii::$app->request->post('PilotFrontIntentionalHighfive')['feature_serial'];
                $linked_feature_id = Yii::$app->request->post('PilotFrontIntentionalHighfive')['linked_feature_id'];
                $checkModel = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontIntentionalHighfive;
                    $model->game_id = $game;
                    $model->challenge_id = $game_id;
                    $model->user_id = $user_id;
                    $model->company_id = $comp_id;
                    ...
                    if (Yii::$app->request->post('hf_image_status')):
                        if (isset($_SESSION['hf_cmnt_img'])):
                            $image_model = new PilotFrontIntentionalHighfive;
...
//              die;
                            ...
                            if ($tagged_uid != $user_id):
                                $notif_model = new PilotFrontIntentionalNotifications;
            ...
            //Leadership Corner
            $leadership_entry_weekly = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
            $leadership_entry_currentday = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
            $leadership_entry_first = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
            $leadership_entry_sec = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
            $leadership_entry_third = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();

            //Weekly Challenge
            $weekly_entry = PilotFrontIntentionalWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();
            ...
            //Share A Win 
            $shareawin_entry = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            //$shareawins_all = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])->joinWith('userinfo')->orderBy(['created' => SORT_DESC])->all();

            $shareawins_all = PilotFrontIntentionalShareawin::find()
                            ->where(['game_id' => $game, 'pilot_front_intentional_shareawin.company_id' => $comp_id, 'pilot_front_intentional_shareawin.challenge_id' => $game_id])
                            ->joinWith('userinfo')
            ...
            if (!empty($team_id)):
                $users_points_obj = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            else:
                $users_points_obj = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            endif;
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontIntentionalSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontIntentionalSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontIntentionalSurveyData')):
                $skipped_entry = PilotFrontIntentionalSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontIntentionalSurveyData');
...
//            $fitness_data = PilotFrontIntentionalStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
//
//            //Congratulation Modal Show on 10K Steps reach per day..
//            $show_10k_congrats = 'false';
//            $steps_obj = PilotFrontIntentionalStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
//            if (!empty($steps_obj)):
//                $total_stepsofday = $steps_obj->steps;
//                if ($total_stepsofday >= 10000):
//                    $check_already_readforday = PilotFrontIntentionalStepsModal::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'modal_read' => 'Yes'])->one();
//                    if (empty($check_already_readforday)):
//                        $show_10k_congrats = 'true';
//                    endif;
//                endif;
//            endif;
            $ratingmodel = new PilotFrontIntentionalRating;
    ...
    public function actionCheckin() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        $shareawins_all = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
    ...
    public function actionHighFive() {
        ...
        $searchModel = new PilotFrontIntentionalHighfiveSearch();
    ...
    public function actionToolbox() {
        ...
        //Leadership Corner
        $leadership_entry_weekly = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
        $leadership_entry_currentday = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        $leadership_entry_first = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
        $leadership_entry_sec = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();
        $leadership_entry_third = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third', 'challenge_id' => $game_id])->one();
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            //All Users Points - Company & Team Wise
            if (!empty($team_id)):
                $points_obj = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'challenge_id' => $game_id])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(10)
                    ->all();
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        endif;

        //Login User Team Total Intentional Values
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        else:
            $login_user_team_core_values = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'challenge_id' => $game_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 20;
        $week_entries = PilotFrontIntentionalDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
        ...
        //Makeup Days entry
        $total_points_dailyInsp = PilotFrontIntentionalDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->sum('points');
        ...
        $daily_model = new PilotFrontIntentionalDailyinspiration;
        $daily_email = new PilotFrontIntentionalEmail;
        $daily_entry = PilotFrontIntentionalDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        if (Yii::$app->request->post()):
            $user_id = $_POST['user_id'];
            $dayset = $_POST['dayset'];
            $checkModel = PilotFrontIntentionalDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontIntentionalDailyinspiration;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    /**
     * 
     * @return Intentional Values Modal
     */
    public function actionCoreValuesModal() {
        ...
        $count_week_entr = 0;
        $points = 5;
        $week_entries = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
        $count_week_entr = count($week_entries);
        if ($count_week_entr >= 5):
            $points = 0;
        endif;
        ...
        //Makeup Days entry
        $total_points_corevalue = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'challenge_id' => $game_id])
                ->sum('points');
        ...
        $corevalues_model = new PilotFrontIntentionalCheckin;
        $corevalues_entry = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        //
        if ($_POST):
            // echo '<pre>';print_r($_POST);die;
            $user_id = Yii::$app->user->identity->id;
            // $dayset = Yii::$app->request->post('PilotFrontIntentionalCheckin')['dayset'];
            $checkModel = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontIntentionalCheckin;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
    ...
    public function actionLeadershipModal() {
        ...
        $leadership_model = new PilotFrontIntentionalLeadershipcorner;
        $leadership_entry = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if (Yii::$app->request->post()):
            $user_id = $_POST['user_id'];
            $tip_pos = $_POST['tip_pos'];
            //$dayset = Yii::$app->request->post('PilotFrontIntentionalLeadershipcorner')['dayset'];
            $checkModel = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontIntentionalLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            if (!empty($game_obj->makeup_days)):
            ...
            if (empty($checkModel)):
                $model = new PilotFrontIntentionalLeadershipcorner;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionWeeklyModal() {
        ...
        $game_id = $game_obj->id;
        $week_no = PilotFrontUser::getGameWeek($game_obj);
        $weekly_entry = PilotFrontIntentionalWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->one();
        $timezone = Yii::$app->user->identity->timezone;
        date_default_timezone_set($timezone);
        $dayset1 = date('Y-m-d');
        if (empty($weekly_entry)):
            $weekly_model = new PilotFrontIntentionalWeeklychallenge;
        else:
        ...
        if (Yii::$app->request->post()):
            ...
            $dayset = date('Y-m-d');
            $comment = $_POST['comment'];
            $checkModel = PilotFrontIntentionalWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();

            //Makeup Days entry            
            $total_points_weeklyvideo = PilotFrontIntentionalWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            if (!empty($game_obj->makeup_days)):
            ...
            if (empty($checkModel)):
                $model = new PilotFrontIntentionalWeeklychallenge;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionShareABinModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $prev_entry_currentday = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
        ...
        //Game Points 5 Days Per Week
        $sum_week_points = 0;
        $week_entries_points = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry       
        $total_points_shareawin = PilotFrontIntentionalShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->sum('points');
        if (!empty($game_obj->makeup_days)):
            if (!empty($total_points_shareawin) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
                ...
                $totalweek = $total_gameday / 7;
                $exactWeek = explode('.', $totalweek);
                $total_game_points = $exactWeek[0] * 10;
        ...
        //end here
        $shareawin_model = new PilotFrontIntentionalShareawin;
        if (Yii::$app->request->post()):
            $comment = $_POST['comment'];
            $user_id = Yii::$app->user->identity->id;
            $model = new PilotFrontIntentionalShareawin;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id, 'challenge_id' => $game_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontIntentionalHighfive;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            ...
            $model->save(false);
            //Save Notification for User in pilot_front_notifications Table
            $linked_feature_user_model = PilotFrontIntentionalHighfive::find()->where(['id' => $linked_feature_id])->one();
        ...
        $total_likes = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontIntentionalHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontIntentionalHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontIntentionalHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontIntentionalHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id, 'challenge_id' => $game_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontIntentionalHighfive;
            $model->challenge_id = $game_id;
        ...
        $allComments = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id, 'challenge_id' => $game_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontIntentionalNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontIntentionalNotifications::find()->where(['id' => $notif_id])->one();
...
//        $digits_count = strlen(count($total_core_values));
        $total_points_data = PilotFrontIntentionalTotalPoints::find()->select('total_points,total_raffle_entry')->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'challenge_id' => $game_id])->one();
...
//        $game_end_date_timestamp = strtotime('+1 day', $game_end_date_timestamp);
        // $game_end_date_timestamp = 1512806400;
        $users_points_obj = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                ->limit(5)
                ->all();
        ...
        $team_core_values_count = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_obj->id])
                ->sum('total_core_values');

        $total_actions_count = PilotFrontIntentionalTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_obj->id])
                ->sum('total_game_actions');
...
//            $list_dates = array_merge($list_start_dates, $list_end_dates);
        ...
        foreach ($list_dates as $index => $val):
            ...
            foreach ($pointsArr as $date => $value):
                if (in_array($date, $gameDates)):
                    ...
                    $check_in_points_res = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $date, 'challenge_id' => $game_id])
                            ->andWhere(['!=', 'label', 'core_values_popup'])
                            ->sum('points');
        ...
        if (Yii::$app->request->post('PilotFrontIntentionalCheckin')):
            $label = Yii::$app->request->post('PilotFrontIntentionalCheckin')['label'];
            $comment = Yii::$app->request->post('PilotFrontIntentionalCheckin')['comment'];
            $serial = Yii::$app->request->post('PilotFrontIntentionalCheckin')['serial'];
            $dayset = Yii::$app->request->post('PilotFrontIntentionalCheckin')['dayset'];
            ...
            //Game Points 5 Days Per Week
            $sum_week_points = 0;
            $week_entries_points = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_asper_date, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
            ...
            $checkModel = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'serial' => $serial, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            if (empty($checkModel)):
                $model = new PilotFrontIntentionalCheckin;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
    ...
    public function actionGetCalRecord() {
        if (Yii::$app->request->isAjax == true) {
            ...
            $valuesLabel = ArrayHelper::map($valuesLabel_obj, 'core_values_name', 'core_values_name');
            $today_valuesModel = new PilotFrontIntentionalCheckin;
            $prev_values_currentday = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset, 'challenge_id' => $game_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
    ...
    public function actionWelcome() {
        ...
        if (!empty($gamemodel)):
            if (((date("m-d-Y", $gamemodel->challenge_start_date)) <= $currentdate)) {
                return $this->redirect(Url::to(['/intentional/dashboard']));
            }
        endif;
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontIntentionalSurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionFitnessSteps() {
        ...
        foreach ($gamedates_array as $key => $gamedate):
            ...
            $fetch_data = PilotFrontIntentionalStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($fetch_data)):
    ...
    public function actionPostApiData() {
        ...
        if (isset($_REQUEST['uid']) && isset($_REQUEST['date']) && isset($_REQUEST['steps']) && isset($_REQUEST['calories']) && isset($_REQUEST['sleephr']) && isset($_REQUEST['distance'])):
            ...
            if (!empty($result_uid) && ($ruid > 0)):
                $getdata = PilotFrontIntentionalStepsApiData::find()->where(['user_id' => $uid, 'date' => $date])->one();
                //Get the Challenge(Game) ID
                $game = PilotFrontUser::getGameID('intentional');
                //User Company ID
                ...
                $week_asper_date = PilotFrontUser::getGameWeekAsPerDate($game_obj, $dayset);
                if (empty($getdata)):
                    $api_model = new PilotFrontIntentionalStepsApiData();
    ...
    public function actionTotalStepsModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $totalsteps_model = new PilotFrontIntentionalStepsApiData();
        //My Total Steps for Complete Challenge
        $user_total_steps = PilotFrontIntentionalStepsApiData::getUserStepsforchallenge($user_id);
        //Top five Users Total Steps for Complete Challenge
        $all_users_total_steps = PilotFrontIntentionalStepsApiData::getAllUsersStepsforchallenge();
    ...
    public function actionReadCongrats() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $congratsModel = new PilotFrontIntentionalStepsModal();
    ...
    public function actionTechSupport() {
        ...
        //Active Challenge Object (Start Date & End Date) 
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $model = new PilotFrontIntentionalTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontIntentionalTechSupport'];
            ...
            $newModel = new PilotFrontIntentionalTechSupport();
    ...
    public function actionUserlist() {
        ...
        $user_idlist = PilotFrontIntentionalHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'challenge_id' => $game_obj->id]);
...
//        }
    ...
    public function actionSeeAll() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontIntentionalNotifications::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])->orderBy(['created' => SORT_DESC]);
        $dataProvider = new ActiveDataProvider([
            'query' => $shareawins_all,
            'pagination' => [
                'pageSize' => 50,
    ...
    public function actionSaveRating() {
        ...
        $html = '';
        if (Yii::$app->request->post()):
            $model = PilotFrontIntentionalRating::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'company_id' => $comp_id])->one();
            if (empty($model)):
                $model = new PilotFrontIntentionalRating;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->company_id = $comp_id;
                $model->user_id = $user_id;
                $model->value = Yii::$app->request->post()['PilotFrontIntentionalRating']['value'];
                $model->dayset = $dayset;
                $model->week_no = $week_no;
                $model->created = time();
                $model->updated = time();
    ...
    public function actionWeekly() {
        ...
        //Leadership Corner
        $leadership_entry_weekly = PilotFrontIntentionalWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
    ...
    public function actionHighFiveimages() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        $subQuery1 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query1 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(5);
        $all_highfiveImage = $query1->all();
        $query2 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
    ...
    public function actionLoadHighfiveImages() {
        ...
        if (count($all_highfiveImage) == 0):
            ...
            foreach ($all_highfiveImage as $hghfv1):
                ...
                $check_comment_liked = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->one();
                ...
                $total_likes = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->all();
                $comment_image = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $hghfv1->id, 'challenge_id' => $game_id])->one();
    ...
    public function actionMoreHfComments() {
        ...
        if (in_array(11, $feature_array)):
            $subQuery = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $query = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count);
            $all_highfivecomment = $query->all();
            $fetched_comment = count($all_highfivecomment);
            $query1 = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $subQuery])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
            $total_comment = $query1->count();
            $image = 0;
        //end here
        else:
            //High Five 
            $all_highfivecomment = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count)->all();
            $fetched_comment = count($all_highfivecomment);
            $query1 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
            $total_comment = $query1->count();
            $image = 1;
//end here
        endif;
...
//        $total_comment = $query1->count();
    ...
    public function actionSaveHighfiveImageComment() {
        ...
        $sub_Query = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $Query = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $prev_highfives_currentday = $Query->all();
        $hf_points = 10;
        $count_hf = 0;
        if (!empty($prev_highfives_currentday)):
        ...
        //Game points for 5s in action
        $subQuery2 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query2 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery2])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $all_highfiveImage1 = $query2->all();
        if (empty($all_highfiveImage1)):
            $pointsimage = 30;
        else:
        ...
        /* $checkModel = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'dayset' => $dayset])->one();
          if (empty($checkModel)): */
        $model = new PilotFrontIntentionalHighfive;
        $model->game_id = $game;
        $model->challenge_id = $game_id;
        ...
        $model->linked_feature_id = $linked_feature_id;
//                    if (Yii::$app->request->post('hf_image_status')):
//                        $type = PilotFrontIntentionalHighfiveType::find()->where(['id' => 2])->one();
//                        $type->type = 1;
//                        $type->created = time();
//                        $type->save(false);
...
//              die;
        ...
        if ($i >= 2 && isset($_SESSION['hf_cmnt_img'])):
            ...
            $subQuery1 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $query1 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['challenge_id' => $game_id])->andWhere(['week_no' => $week_no])->andWhere(['user_id' => $user_id])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        ...
        elseif ($i <= 2):
            ...
            $output['serial'] = $feature_serial;
            $sub_Query = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
            $Query = PilotFrontIntentionalHighfive::find()->where(['not in', 'id', $sub_Query])->andWhere(['challenge_id' => $game_id])->andWhere(['=', 'feature_label', 'highfiveComment'])->andWhere(['user_id' => $user_id])->andWhere(['dayset' => $dayset])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
            $prev_highfives_currentday = $Query->all();
            $count_hf = 0;
            if (!empty($prev_highfives_currentday)):
                $count_hf = count($prev_highfives_currentday);
    ...
    public function actionLoadmoreHighfiveImages() {
        ...
        $user_id = Yii::$app->user->identity->id;
        $subQuery1 = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query1 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(5)->offset($offset);
        $all_highfiveImage = $query1->all();
        $query2 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
        $total_comment = $query2->count();
        $count1 = $offset + 5;
        $query3 = PilotFrontIntentionalHighfive::find()->where(['in', 'id', $subQuery1])->andWhere(['=', 'feature_label', 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count1);
    ...
    public function actionSaveHighfiveComment() {
        ...
        $user_id = Yii::$app->user->identity->id;
        $team_id = Yii::$app->user->identity->team_id;
        $prev_highfives_currentday = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
        $hf_points = 10;
        $count_hf = 0;
        if (!empty($prev_highfives_currentday)):
            $count_hf = count($prev_highfives_currentday);
        ...
        //Game Points 5 Days Per Week - Shout Out Section
        $sum_week_points = 0;
        $week_entries_points = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry           
        $total_points_highfive = PilotFrontIntentionalHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->sum('points');
        if (!empty($total_points_highfive) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
            $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
            //echo $total_gameday;
...
//              die;
    ...
    public function actionCoreData() {
        ...
        $prev_today_values_currentday = PilotFrontIntentionalCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
                        ->andWhere(['!=', 'label', 'core_values_popup'])->orderBy(['created' => SORT_DESC])->all();
        $count_tv = 0;
        if (!empty($prev_today_values_currentday)):
            $count_tv = count($prev_today_values_currentday);
