...
use frontend\models\PilotFrontLeadershipDailyinspiration;
use frontend\models\PilotFrontLeadershipLeadershipcorner;
use frontend\models\PilotFrontLeadershipWeeklychallenge;
use frontend\models\PilotFrontLeadershipShareawin;
use frontend\models\PilotFrontLeadershipHighfive;
use frontend\models\PilotFrontLeadershipHighfiveSearch;
use frontend\models\PilotFrontLeadershipCheckin;
use frontend\models\PilotFrontUser;
use frontend\models\PilotFrontLeadershipNotifications;
use frontend\models\PilotFrontLeadershipTotalPoints;
use frontend\models\PilotFrontLeadershipSurveyData;
use frontend\models\PilotFrontLeadershipStepsApiData;
use frontend\models\PilotFrontLeadershipStepsApiDataSearch;
use frontend\models\PilotFrontLeadershipStepsModal;
use frontend\models\PilotFrontLeadershipTechSupport;
...
use frontend\models\PilotFrontLeadershipEmail;
...
class LeadershipController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '4' && $challenge_id != 0) {
...
//            $session->set('company_id', $company_obj->id);
        //return $this->redirect('http://' . $check_comp . '.injoychange.com/leadership/dashboard');
        endif;
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                if (in_array($team_id, $challenge_teams)):
                    ...
                    return $this->redirect(Url::to(['/leadership/welcome']));
                else:
                    return $this->redirect(Url::to(['/leadership/thankyou']));
                    return $this->goBack();
                endif;
            else:
                return $this->redirect(Url::to(['/leadership/thankyou']));
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontLeadershipDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            //Core Values
            ...
            $corevalues_entry = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => 'core_values_popup', 'dayset' => $dayset])->one();
            $prev_today_values_currentday = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
            ...
            $total_weeks_entries_points = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum(points);
            ...
            $today_valuesModel = new PilotFrontLeadershipCheckin;
            if (Yii::$app->request->post('PilotFrontLeadershipCheckin')):
                $label = Yii::$app->request->post('PilotFrontLeadershipCheckin')['label'];
                $comment = Yii::$app->request->post('PilotFrontLeadershipCheckin')['comment'];
                $serial = Yii::$app->request->post('PilotFrontLeadershipCheckin')['serial'];
                $checkModel = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'serial' => $serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontLeadershipCheckin;
                    $model->game_id = $game;
            ...
            //High Five 
            $all_highfives = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(40)->all();
            $prev_highfives_currentday = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
            $hf_points = 10;
            $count_hf = 0;
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
            endif;
            ...
            //Makeup Days entry           
            $total_points_highfive = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            $highfiveModel = new PilotFrontLeadershipHighfive;
            if (Yii::$app->request->post('PilotFrontLeadershipHighfive')):
                $feature_label = Yii::$app->request->post('PilotFrontLeadershipHighfive')['feature_label'];
                $feature_value = Yii::$app->request->post('PilotFrontLeadershipHighfive')['feature_value'];
                $feature_serial = Yii::$app->request->post('PilotFrontLeadershipHighfive')['feature_serial'];
                $linked_feature_id = Yii::$app->request->post('PilotFrontLeadershipHighfive')['linked_feature_id'];
                $checkModel = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontLeadershipHighfive;
                    $model->game_id = $game;
                    $model->challenge_id = $game_id;
                    $model->user_id = $user_id;
                    $model->company_id = $comp_id;
                    ...
                    if (Yii::$app->request->post('hf_image_status')):
                        if (isset($_SESSION['hf_cmnt_img'])):
                            $image_model = new PilotFrontLeadershipHighfive;
...
//              die;
                            ...
                            if ($tagged_uid != $user_id):
                                $notif_model = new PilotFrontLeadershipNotifications;
            ...
            //Leadership Corner
            $leadership_entry_weekly = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
            $leadership_entry_currentday = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
            $leadership_entry_first = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
            $leadership_entry_sec = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
            $leadership_entry_third = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();

            //Weekly Challenge
            $weekly_entry = PilotFrontLeadershipWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();
            ...
            //Share A Win 
            $shareawin_entry = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->all();
            //$shareawins_all = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])->joinWith('userinfo')->orderBy(['created' => SORT_DESC])->all();

            $shareawins_all = PilotFrontLeadershipShareawin::find()
                            ->where(['game_id' => $game, 'pilot_front_leadership_shareawin.company_id' => $comp_id])
                            ->joinWith('userinfo')
            ...
            if (!empty($team_id)):
                $users_points_obj = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            else:
                $users_points_obj = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            endif;
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontLeadershipSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontLeadershipSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontLeadershipSurveyData')):
                $skipped_entry = PilotFrontLeadershipSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontLeadershipSurveyData');
            ...
            $fitness_data = PilotFrontLeadershipStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();

            //Congratulation Modal Show on 10K Steps reach per day..
            $show_10k_congrats = 'false';
            $steps_obj = PilotFrontLeadershipStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($steps_obj)):
                $total_stepsofday = $steps_obj->steps;
                if ($total_stepsofday >= 10000):
                    $check_already_readforday = PilotFrontLeadershipStepsModal::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'modal_read' => 'Yes'])->one();
                    if (empty($check_already_readforday)):
                        $show_10k_congrats = 'true';
                    endif;
                endif;
            endif;
            $ratingmodel = new PilotFrontLeadershipRating;
    ...
    public function actionCheckin() {
        ...
        $game = PilotFrontUser::getGameID('leadership');
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
    ...
    public function actionHighFive() {
        ...
        $searchModel = new PilotFrontLeadershipHighfiveSearch();
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            //All Users Points - Company & Team Wise
            if (!empty($team_id)):
                $points_obj = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(10)
                    ->all();
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        endif;

        //Login User Team Total Leadership Values
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        else:
            $login_user_team_core_values = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 10;
        $week_entries = PilotFrontLeadershipDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        ...
        //Makeup Days entry
        $total_points_dailyInsp = PilotFrontLeadershipDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                ->sum('points');
        ...
        $daily_model = new PilotFrontLeadershipDailyinspiration;
        $daily_email = new PilotFrontLeadershipEmail;
        $daily_entry = PilotFrontLeadershipDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontLeadershipDailyinspiration')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontLeadershipDailyinspiration')['dayset'];
            $checkModel = PilotFrontLeadershipDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontLeadershipDailyinspiration;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionCoreValuesModal() {
        ...
        $count_week_entr = 0;
        $points = 5;
        $week_entries = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'week_no' => $week_no])->all();
        $count_week_entr = count($week_entries);
        if ($count_week_entr >= 5):
            $points = 0;
        endif;
        ...
        //Makeup Days entry
        $total_points_corevalue = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label])
                ->sum('points');
        ...
        $corevalues_model = new PilotFrontLeadershipCheckin;
        $corevalues_entry = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontLeadershipCheckin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontLeadershipCheckin')['dayset'];
            $checkModel = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontLeadershipCheckin;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
    ...
    public function actionLeadershipModal() {
        ...
        $leadership_model = new PilotFrontLeadershipLeadershipcorner;
        $leadership_entry = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontLeadershipLeadershipcorner')['user_id'];
            $tip_pos = Yii::$app->request->post('PilotFrontLeadershipLeadershipcorner')['tip_pos'];
            $dayset = Yii::$app->request->post('PilotFrontLeadershipLeadershipcorner')['dayset'];
            $checkModel = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontLeadershipLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontLeadershipLeadershipcorner;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionWeeklyModal() {
        ...
        if (empty($weekly_entry)):
            $weekly_model = new PilotFrontLeadershipWeeklychallenge;
        else:
            $weekly_model = $weekly_entry;
        endif;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontLeadershipWeeklychallenge')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontLeadershipWeeklychallenge')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontLeadershipWeeklychallenge')['comment'];
            $checkModel = PilotFrontLeadershipWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();

            //Makeup Days entry            
            $total_points_weeklyvideo = PilotFrontLeadershipWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontLeadershipWeeklychallenge;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionShareABinModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $prev_entry_currentday = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
        ...
        //Game Points 5 Days Per Week
        $sum_week_points = 0;
        $week_entries_points = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry       
        $total_points_shareawin = PilotFrontLeadershipShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                ->sum('points');
        ...
        //end here
        $shareawin_model = new PilotFrontLeadershipShareawin;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontLeadershipShareawin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontLeadershipShareawin')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontLeadershipShareawin')['comment'];
            $model = new PilotFrontLeadershipShareawin;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontLeadershipHighfive;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            ...
            $model->save(false);
            //Save Notification for User in pilot_front_notifications Table
            $linked_feature_user_model = PilotFrontLeadershipHighfive::find()->where(['id' => $linked_feature_id])->one();
        ...
        $total_likes = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontLeadershipHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontLeadershipHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontLeadershipHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontLeadershipHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontLeadershipHighfive;
            $model->challenge_id = $game_id;
        ...
        $allComments = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontLeadershipHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontLeadershipNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontLeadershipNotifications::find()->where(['id' => $notif_id])->one();
    ...
    public function actionGetUserPoints() {
        ...
        //Count the Total Leadership Values of the User
        $total_core_values = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                        ->andWhere(['!=', 'label', 'core_values_popup'])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                        ->orderBy(['created' => SORT_DESC])->all();
        ...
        if ($game_obj->id == '159' || $game_obj->id == '158'):
            $total_steps = PilotFrontLeadershipStepsApiData::getUserStepsforchallenge($user_id);
        ...
        //Save the User Points to the Main Table of All Users Points of Leadership Challenge
        $check_entry = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->one();
        if (empty($check_entry)):
            $model = new PilotFrontLeadershipTotalPoints;
            $model->game_id = $game;
            $model->challenge_id = $game_id;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionGetCompanyUsersPoints() {
        ...
        $team_core_values_count = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_core_values');

        $total_actions_count = PilotFrontLeadershipTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_game_actions');
...
//        endif;
        ...
        foreach ($list_dates as $index => $val):
            ...
            foreach ($pointsArr as $date => $value):
                if (in_array($date, $gameDates)):
                    ...
                    $check_in_points_res = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $date])
                            ->andWhere(['!=', 'label', 'core_values_popup'])
                            ->sum('points');
        ...
        if (Yii::$app->request->post('PilotFrontLeadershipCheckin')):
            ...
            //Game Points 5 Days Per Week
            $sum_week_points = 0;
            $week_entries_points = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_asper_date])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
    ...
    public function actionGetCalRecord() {
        if (Yii::$app->request->isAjax == true) {
            ...
            $valuesLabel = ArrayHelper::map($valuesLabel_obj, 'core_values_name', 'core_values_name');
            $today_valuesModel = new PilotFrontLeadershipCheckin;
            $prev_values_currentday = PilotFrontLeadershipCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
    ...
    public function actionWelcome() {
        ...
        if (!empty($gamemodel)):
            if (((date("m-d-Y", $gamemodel->challenge_start_date)) <= $currentdate)) {
                return $this->redirect(Url::to(['/leadership/dashboard']));
            }
        endif;
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontLeadershipSurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionFitnessSteps() {
        ...
        foreach ($gamedates_array as $key => $gamedate):
            ...
            $fetch_data = PilotFrontLeadershipStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($fetch_data)):
    ...
    public function actionPostApiData() {
        ...
        if (isset($_REQUEST['uid']) && isset($_REQUEST['date']) && isset($_REQUEST['steps']) && isset($_REQUEST['calories']) && isset($_REQUEST['sleephr']) && isset($_REQUEST['distance'])):
            ...
            if (!empty($result_uid) && ($ruid > 0)):
                $getdata = PilotFrontLeadershipStepsApiData::find()->where(['user_id' => $uid, 'date' => $date])->one();
                //Get the Challenge(Game) ID
                $game = PilotFrontUser::getGameID('leadership');
                //User Company ID
                ...
                $week_asper_date = PilotFrontUser::getGameWeekAsPerDate($game_obj, $dayset);
                if (empty($getdata)):
                    $api_model = new PilotFrontLeadershipStepsApiData();
    ...
    public function actionTotalStepsModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $totalsteps_model = new PilotFrontLeadershipStepsApiData();
        //My Total Steps for Complete Challenge
        $user_total_steps = PilotFrontLeadershipStepsApiData::getUserStepsforchallenge($user_id);
        //Top five Users Total Steps for Complete Challenge
        $all_users_total_steps = PilotFrontLeadershipStepsApiData::getAllUsersStepsforchallenge();
    ...
    public function actionReadCongrats() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $congratsModel = new PilotFrontLeadershipStepsModal();
    ...
    public function actionTechSupport() {
        ...
        //Active Challenge Object (Start Date & End Date) 
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $model = new PilotFrontLeadershipTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontLeadershipTechSupport'];
            ...
            $newModel = new PilotFrontLeadershipTechSupport();
    ...
    public function actionUserlist() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $challenge_id = Yii::$app->user->identity->challenge_id;
        $user_idlist = PilotFrontLeadershipHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'game_id' => $challenge_id]);
...
//        }
    ...
    public function actionSeeAll() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontLeadershipNotifications::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])->orderBy(['created' => SORT_DESC]);
        $dataProvider = new ActiveDataProvider([
            'query' => $shareawins_all,
            'pagination' => [
                'pageSize' => 50,
    ...
    public function actionDailyEmail() {
        ...
        if (Yii::$app->request->post()):
            ...
            $send_email = Yii::$app
                    ->mailer
                    ->compose('send-image', ['model' => Yii::$app->request->post()['PilotFrontLeadershipEmail']['message'], 'daily_image_path' => $daily_image_path])
                    ->setFrom('support@injoyglobal.com')
                    ->setTo(Yii::$app->request->post()['PilotFrontLeadershipEmail']['email_id'])
                    ->setSubject('Daily Inspiration Image')
                    ->send();
//            $message = Yii::$app->mailer->compose('send-image', ['model' => Yii::$app->request->post()['PilotFrontLeadershipEmail']['message'], 'daily_image_path' => $daily_image_path])
//                    ->setFrom('support@injoyglobal.com')
//                    ->setTo(Yii::$app->request->post()['PilotFrontLeadershipEmail']['email_id'])
//                    ->setSubject('Daily Inspiration Image');
//            $message->send();
//            $to = Yii::$app->request->post()['PilotFrontLeadershipEmail']['email_id'];
...
//                $daily_email->email_id = Yii::$app->request->post()['PilotFrontLeadershipEmail']['email_id'];
...
//            }
    ...
    public function actionSaveRating() {
        ...
        $html = '';
        if (Yii::$app->request->post()):
            $model = PilotFrontLeadershipRating::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'company_id' => $comp_id])->one();
            if (empty($model)):
                $model = new PilotFrontLeadershipRating;
                $model->challenge_id = $game_obj->id;
                $model->game_id = $game;
                $model->company_id = $comp_id;
                $model->user_id = $user_id;
                $model->value = Yii::$app->request->post()['PilotFrontLeadershipRating']['value'];
                $model->dayset = $dayset;
                $model->week_no = $week_no;
                $model->created = time();
                $model->updated = time();
