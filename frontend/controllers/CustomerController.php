...
use frontend\models\PilotFrontCustomerDailyinspiration;
use frontend\models\PilotFrontCustomerHighfive;
use frontend\models\PilotFrontCustomerHighfiveSearch;
use frontend\models\PilotFrontUser;
use frontend\models\PilotFrontCustomerNotifications;
use frontend\models\PilotFrontCustomerTotalPoints;
use frontend\models\PilotFrontCustomerSurveyData;
use frontend\models\PilotFrontCustomerTechSupport;
use frontend\models\PilotFrontCustomerWeeklychallenge;
use frontend\models\PilotFrontCustomerKnowcorner;
...
use frontend\models\PilotFrontCustomerEmail;
...
class CustomerController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '8' && $challenge_id != 0) {
...
//            $session->set('company_id', $company_obj->id);
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                if (in_array($team_id, $challenge_teams)):
                    ...
                    return $this->redirect(Url::to(['/customer/welcome']));
                else:
                    return $this->redirect(Url::to(['/customer/thankyou']));
                    return $this->goBack();
                endif;
            else:
                return $this->redirect(Url::to(['/customer/thankyou']));
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontCustomerDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            $all_highfives = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(20)->all();
            $prev_highfives_currentday = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
            $hf_points = 10;
            $count_hf = 0;
            ...
            $highfiveModel = new PilotFrontCustomerHighfive;
            //Audio Challenge

            $audio_entry_weekly = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $audio_entry_currentday = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $audio_entry_first = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $audio_entry_sec = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();
            $audio_entry_third = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third', 'challenge_id' => $game_id])->one();

            // did you know corner

            $know_entry_weekly = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $know_entry_currentday = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $know_entry_first = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $know_entry_sec = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();

            // Question Corner

            $answer_entry_weekly = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontCustomerSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontCustomerSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontCustomerSurveyData')):
                $skipped_entry = PilotFrontCustomerSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontCustomerSurveyData');
    ...
    public function actionHighFive() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $searchModel = new PilotFrontCustomerHighfiveSearch();
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 10;
        $week_entries = PilotFrontCustomerDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        ...
        if (!empty($game_obj->makeup_days)):
            $total_points_dailyInsp = PilotFrontCustomerDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
        ...
        $daily_model = new PilotFrontCustomerDailyinspiration;
        $daily_email = new PilotFrontCustomerEmail;
        $daily_entry = PilotFrontCustomerDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = $_POST['user_id'];
            $dayset = $_POST['dayset'];
            $checkModel = PilotFrontCustomerDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontCustomerDailyinspiration;
                $model->game_id = $game;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontCustomerHighfive;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
            ...
            $linked_feature_user_model = PilotFrontCustomerHighfive::find()->where(['id' => $linked_feature_id])->one();
            ...
            if ($linked_feature_user->id != $user_id):
                $notif_model = new PilotFrontCustomerNotifications;
                $model->challenge_id = $game_id;
        ...
        $total_likes = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontCustomerHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontCustomerHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontCustomerHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontCustomerHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontCustomerHighfive;
            $model->challenge_id = $game_id;
        ...
        $allComments = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontCustomerNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontCustomerNotifications::find()->where(['id' => $notif_id])->one();
    ...
    public function actionWelcome() {
        ...
        if (!empty($gamemodel)):
            if (((date("m-d-Y", $gamemodel->challenge_start_date)) <= $currentdate)) {
                return $this->redirect(Url::to(['/customer/dashboard']));
            }
        endif;
    ...
    public function actionAudioModal() {
        ...
        $audio_model = new PilotFrontCustomerWeeklychallenge;
        $audio_entry = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if ($_POST):
            $user_id = Yii::$app->user->identity->id;
            ...
            $answer = $_POST['answer'];
            $dayset = $dayset;
            $checkModel = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('points');
            if (!empty($game_obj->makeup_days)):
            ...
            //end here
            if (empty($checkModel)):
                $model = new PilotFrontCustomerWeeklychallenge;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontCustomerSurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionTechSupport() {
        ...
        //Active Challenge Object (Start Date & End Date) 
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $model = new PilotFrontCustomerTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontCustomerTechSupport'];
            ...
            $newModel = new PilotFrontCustomerTechSupport();
    ...
    public function actionSaveHighfiveImageComment() {
        ...
        $sum_week_points = 0;
        $week_entries_points = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        ...
        //Makeup Days entry           
        $total_points_highfive = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->sum('points');
        if (!empty($game_obj->makeup_days)):
        ...
        /* $checkModel = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'dayset' => $dayset])->one();
          if (empty($checkModel)): */
        $model = new PilotFrontCustomerHighfive;
        $model->game_id = $game;
        $model->challenge_id = $game_id;
        ...
        if (isset($_SESSION['hf_cmnt_img'])):
            $image_model = new PilotFrontCustomerHighfive;
...
//              die;
    ...
    public function actionMoreHfComments() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        //$subQuery = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count);
        $all_highfivecomment = $query->all();
        $fetched_comment = count($all_highfivecomment);
        $query1 = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
    ...
    public function actionUserlist() {
        $search = $_POST['query'];
        $comp_id = Yii::$app->user->identity->company_id;
        $challenge_id = PilotFrontUser::getGameID('customer');
        $user_idlist = PilotFrontCustomerHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'game_id' => $challenge_id]);
...
//        }
    ...
    public function actionKnowModal() {
        ...
        $dayset = date('Y-m-d');
        $tip_pos = Yii::$app->request->get()['tip'];
        $know_model = new PilotFrontCustomerKnowcorner;
        $know_entry = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if ($_POST):
            $user_id = Yii::$app->user->identity->id;
            $tip_pos = $_POST['tip_pos'];
            $dayset = $dayset;
            $checkModel = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontCustomerKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('points');
            if (!empty($game_obj->makeup_days)):
                if (!empty($total_points_leadership) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
                    $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
            ...
            //end here
            if (empty($checkModel)):
                $model = new PilotFrontCustomerKnowcorner;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionGetUserPoints() {
        ...
        $check_entry = PilotFrontCustomerTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->one();
        if (empty($check_entry)):
            $model = new PilotFrontCustomerTotalPoints;
            $model->game_id = $game;
            $model->challenge_id = $game_id;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
    ...
    public function actionToolbox() {
        ...
        //Leadership Corner
        $leadership_entry_weekly = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        $leadership_entry_currentday = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
        $leadership_entry_first = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
        $leadership_entry_sec = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
        $leadership_entry_third = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();
    ...
    public function actionKnowTheTeam() {
        ...
        $leadership_tips = PilotGettoknowCorner::find()->where(['category_id' => $knowtheteam])
                ...
                ->all();
        $week_pattern = $week_leadership . '%';
        $leadership_entry_weekly = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        $leadership_entry_currentday = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
        $leadership_entry_first = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
        $leadership_entry_sec = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
        $leadership_entry_third = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();
...
//            $tip = $tip_no3;
//        endif;
// $answer_entry_second = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            ...
            if (!empty($team_id)):
                $points_obj = PilotFrontCustomerTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                        ->andWhere(['!=','user_id',691])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontCustomerTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['!=','user_id',691])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(20)
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontCustomerTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontCustomerTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                            ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])->count('id');
            $login_user_team_core_values = PilotFrontCustomerWeeklychallenge::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                            ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontCustomerHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
    ...
    public function actionWeeklyModal() {
        ...
        $game_id = $game_obj->id;
        $week_no = PilotFrontUser::getGameWeek($game_obj);
        $weekly_entry = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();
        $timezone = Yii::$app->user->identity->timezone;
        //date_default_timezone_set($timezone);
        $dayset1 = date('Y-m-d');
//        if (empty($weekly_entry)):
        $weekly_model = new PilotFrontCustomerQuestion;
//        else:
...
//        endif;
        if (Yii::$app->request->post()):
            ...
            $dayset = date('Y-m-d');
            $comment = $_POST['comment'];
            $checkModel = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->one();
            ...
            //Makeup Days entry            
            $total_points_weeklyvideo = PilotFrontCustomerQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            if (!empty($game_obj->makeup_days)):
            ...
            //end here
            // if (empty($checkModel)):
            $model = new PilotFrontCustomerQuestion;
            $model->game_id = $game;
            $model->challenge_id = $game_id;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
...
//                endif;
    ...
    public function actionGenerateGraph(){
