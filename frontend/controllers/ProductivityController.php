...
use frontend\models\PilotFrontProductivityDailyinspiration;
use frontend\models\PilotFrontProductivityLeadershipcorner;
use frontend\models\PilotFrontProductivityWeeklychallenge;
use frontend\models\PilotFrontProductivityShareawin;
use frontend\models\PilotFrontProductivityHighfive;
use frontend\models\PilotFrontProductivityHighfiveSearch;
use frontend\models\PilotFrontProductivityCheckin;
use frontend\models\PilotFrontUser;
use frontend\models\PilotFrontProductivityNotifications;
use frontend\models\PilotFrontProductivityTotalPoints;
use frontend\models\PilotFrontProductivitySurveyData;
use frontend\models\PilotFrontProductivityStepsApiData;
use frontend\models\PilotFrontProductivityStepsApiDataSearch;
use frontend\models\PilotFrontProductivityStepsModal;
use frontend\models\PilotFrontProductivityTechSupport;
use frontend\models\PilotFrontProductivityEmail;
use frontend\models\PilotFrontProductivityRating;
...
/**
 * ProductivityController implements the CRUD actions for Productivity model.
 */
class ProductivityController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '3' && $challenge_id != 0) {
...
//            $session->set('company_id', $company_obj->id);
        //return $this->redirect('http://' . $check_comp . '.injoychange.com/productivity/dashboard');
        endif;
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                if (in_array($team_id, $challenge_teams)):
                    ...
                    return $this->redirect(Url::to(['/productivity/welcome']));
                else:
                    return $this->redirect(Url::to(['/productivity/thankyou']));
                    return $this->goBack();
                endif;
            else:
                return $this->redirect(Url::to(['/productivity/thankyou']));
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontProductivityDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            //Core Values
            ...
            $corevalues_entry = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => 'core_values_popup', 'dayset' => $dayset])->one();
            $prev_today_values_currentday = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
            ...
            $total_weeks_entries_points = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum(points);
            ...
            $today_valuesModel = new PilotFrontProductivityCheckin;
            if (Yii::$app->request->post('PilotFrontProductivityCheckin')):
                $label = Yii::$app->request->post('PilotFrontProductivityCheckin')['label'];
                $comment = Yii::$app->request->post('PilotFrontProductivityCheckin')['comment'];
                $serial = Yii::$app->request->post('PilotFrontProductivityCheckin')['serial'];
                $checkModel = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'serial' => $serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontProductivityCheckin;
                    $model->game_id = $game;
            ...
            //High Five 
            $all_highfives = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(40)->all();
            $prev_highfives_currentday = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
            $hf_points = 10;
            $count_hf = 0;
            ...
            $sum_week_points = 0;
            $week_entries_points = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
            endif;
            ...
            //Makeup Days entry           
            $total_points_highfive = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            $highfiveModel = new PilotFrontProductivityHighfive;
            if (Yii::$app->request->post('PilotFrontProductivityHighfive')):
                $feature_label = Yii::$app->request->post('PilotFrontProductivityHighfive')['feature_label'];
                $feature_value = Yii::$app->request->post('PilotFrontProductivityHighfive')['feature_value'];
                $feature_serial = Yii::$app->request->post('PilotFrontProductivityHighfive')['feature_serial'];
                $linked_feature_id = Yii::$app->request->post('PilotFrontProductivityHighfive')['linked_feature_id'];
                $checkModel = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'dayset' => $dayset])->one();
                if (empty($checkModel)):
                    $model = new PilotFrontProductivityHighfive;
                    $model->game_id = $game;
                    $model->user_id = $user_id;
                    $model->company_id = $comp_id;
                    ...
                    if (Yii::$app->request->post('hf_image_status')):
                        if (isset($_SESSION['hf_cmnt_img'])):
                            $image_model = new PilotFrontProductivityHighfive;
...
//              die;
                            ...
                            if ($tagged_uid != $user_id):
                                $notif_model = new PilotFrontProductivityNotifications;
            ...
            //Leadership Corner
            $leadership_entry_weekly = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
            $leadership_entry_currentday = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
            $leadership_entry_first = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
            $leadership_entry_sec = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
            $leadership_entry_third = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();

            //Weekly Challenge
            $weekly_entry = PilotFrontProductivityWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();
            ...
            if(empty($weekly_challenge_obj)):
                ...
                $game_current_week_no = PilotFrontUser::getvideoweek($game_current_week_no,$weekly_challenge_count);
                $weekly_challenge_obj = PilotWeeklyChallenge::find()->where(['category_id' => $weekly_category_id, 'week' => $game_current_week_no])->one();
            endif;
            ...
            //Share A Win 
            $shareawin_entry = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->all();
            //$shareawins_all = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])->joinWith('userinfo')->orderBy(['created' => SORT_DESC])->all();

            $shareawins_all = PilotFrontProductivityShareawin::find()
                            ->where(['game_id' => $game, 'pilot_front_productivity_shareawin.company_id' => $comp_id])
                            ->joinWith('userinfo')
            ...
            if (!empty($team_id)):
                $users_points_obj = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            else:
                $users_points_obj = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                                ->orderBy(['total_points' => SORT_DESC])->all();
            endif;
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontProductivitySurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontProductivitySurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontProductivitySurveyData')):
                $skipped_entry = PilotFrontProductivitySurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontProductivitySurveyData');
                ...
                foreach ($survey_posted_questions as $key => $q_id):
                    $surveyModel = new PilotFrontProductivitySurveyData();
            ...
            $fitness_data = PilotFrontProductivityStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();

            //Congratulation Modal Show on 10K Steps reach per day..
            $show_10k_congrats = 'false';
            $steps_obj = PilotFrontProductivityStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($steps_obj)):
                $total_stepsofday = $steps_obj->steps;
                if ($total_stepsofday >= 10000):
                    $check_already_readforday = PilotFrontProductivityStepsModal::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'modal_read' => 'Yes'])->one();
            ...
            $ratingmodel = new PilotFrontProductivityRating;
    ...
    public function actionCheckin() {
        ...
        $game = PilotFrontUser::getGameID('productivity');
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
    ...
    public function actionHighFive() {
        ...
        $searchModel = new PilotFrontProductivityHighfiveSearch();
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            //All Users Points - Company & Team Wise
            if (!empty($team_id)):
                $points_obj = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(10)
                    ->all();
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        endif;

        //Login User Team Total Productivity Values
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        else:
            $login_user_team_core_values = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_core_values');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 10;
        $week_entries = PilotFrontProductivityDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        $count_week_entr = count($week_entries);
        $daily_email = new PilotFrontProductivityEmail;
        ...
        //Makeup Days entry
        $total_points_dailyInsp = PilotFrontProductivityDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                ->sum('points');
        ...
        $daily_model = new PilotFrontProductivityDailyinspiration;
        $daily_entry = PilotFrontProductivityDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontProductivityDailyinspiration')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontProductivityDailyinspiration')['dayset'];
            $checkModel = PilotFrontProductivityDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontProductivityDailyinspiration;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
                $model->points = $points;
            ...
            if(empty($daily_inspiration_obj)):
                ...
                $game_current_day_no = PilotFrontUser::getdays($game_current_day_no,$daily_inspiration_count);
                $daily_inspiration_obj = PilotDailyinspirationImage::find()->where(['category_id' => $daily_category_id, 'order_number' => $game_current_day_no])->one();
            endif;
    ...
    public function actionCoreValuesModal() {
        ...
        $count_week_entr = 0;
        $points = 5;
        $week_entries = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'week_no' => $week_no])->all();
        $count_week_entr = count($week_entries);
        if ($count_week_entr >= 5):
            $points = 0;
        endif;
        ...
        //Makeup Days entry
        $total_points_corevalue = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label])
                ->sum('points');
        ...
        $corevalues_model = new PilotFrontProductivityCheckin;
        $corevalues_entry = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontProductivityCheckin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontProductivityCheckin')['dayset'];
            $checkModel = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'label' => $label, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontProductivityCheckin;
                $model->game_id = $game;
                $model->user_id = $user_id;
    ...
    public function actionLeadershipModal() {
        ...
        $leadership_model = new PilotFrontProductivityLeadershipcorner;
        $leadership_entry = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos])->one();
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontProductivityLeadershipcorner')['user_id'];
            $tip_pos = Yii::$app->request->post('PilotFrontProductivityLeadershipcorner')['tip_pos'];
            $dayset = Yii::$app->request->post('PilotFrontProductivityLeadershipcorner')['dayset'];
            $checkModel = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontProductivityLeadershipcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontProductivityLeadershipcorner;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
                $model->week_no = $week_no;
    ...
    public function actionWeeklyModal() {
        ...
        if (empty($weekly_entry)):
            $weekly_model = new PilotFrontProductivityWeeklychallenge;
        else:
            $weekly_model = $weekly_entry;
        endif;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontProductivityWeeklychallenge')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontProductivityWeeklychallenge')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontProductivityWeeklychallenge')['comment'];
            $checkModel = PilotFrontProductivityWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])->one();

            //Makeup Days entry            
            $total_points_weeklyvideo = PilotFrontProductivityWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                    ->sum('points');
            ...
            if (empty($checkModel)):
                $model = new PilotFrontProductivityWeeklychallenge;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
                $model->points = $points;
    ...
    public function actionShareABinModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $prev_entry_currentday = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
        ...
        //Game Points 5 Days Per Week
        $sum_week_points = 0;
        $week_entries_points = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        if (!empty($week_entries_points)):
            $sum_week_points = $week_entries_points;
        endif;
        ...
        //Makeup Days entry       
        $total_points_shareawin = PilotFrontProductivityShareawin::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
                ->sum('points');
        ...
        //end here
        $shareawin_model = new PilotFrontProductivityShareawin;
        if (Yii::$app->request->post()):
            $user_id = Yii::$app->request->post('PilotFrontProductivityShareawin')['user_id'];
            $dayset = Yii::$app->request->post('PilotFrontProductivityShareawin')['dayset'];
            $comment = Yii::$app->request->post('PilotFrontProductivityShareawin')['comment'];
            $model = new PilotFrontProductivityShareawin;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
            $model->team_id = $team_id;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontProductivityHighfive;
            $model->game_id = $game;
            $model->user_id = $user_id;
            ...
            $model->save(false);
            //Save Notification for User in pilot_front_notifications Table
            $linked_feature_user_model = PilotFrontProductivityHighfive::find()->where(['id' => $linked_feature_id])->one();
        ...
        $total_likes = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontProductivityHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontProductivityHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontProductivityHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontProductivityHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontProductivityHighfive;
            $model->game_id = $game;
        ...
        $allComments = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontProductivityHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontProductivityNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontProductivityNotifications::find()->where(['id' => $notif_id])->one();
    ...
    public function actionGetUserPoints() {
        ...
        //Count the Total Productivity Values of the User
        $total_core_values = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                        ->andWhere(['!=', 'label', 'core_values_popup'])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                        ->orderBy(['created' => SORT_DESC])->all();
        ...
        if ($game_obj->id == '159' || $game_obj->id == '158'):
            $total_steps = PilotFrontProductivityStepsApiData::getUserStepsforchallenge($user_id);
        ...
        //Save the User Points to the Main Table of All Users Points of Productivity Challenge
        $check_entry = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->one();
        if (empty($check_entry)):
            $model = new PilotFrontProductivityTotalPoints;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
            $model->team_id = $team_id;
    ...
    public function actionGetCompanyUsersPoints() {
        ...
        $team_core_values_count = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_core_values');

        $total_actions_count = PilotFrontProductivityTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                ->sum('total_game_actions');
    ...
    public function actionCalendar() {
        ...
        foreach ($list_dates as $index => $val):
            ...
            foreach ($pointsArr as $date => $value):
                if (in_array($date, $gameDates)):
                    ...
                    $check_in_points_res = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $date])
                            ->andWhere(['!=', 'label', 'core_values_popup'])
                            ->sum('points');
        ...
        if (Yii::$app->request->post('PilotFrontProductivityCheckin')):
            ...
            //Game Points 5 Days Per Week
            $sum_week_points = 0;
            $week_entries_points = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_asper_date])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
                    ->sum('points');
            if (!empty($week_entries_points)):
                $sum_week_points = $week_entries_points;
    ...
    public function actionGetCalRecord() {
        if (Yii::$app->request->isAjax == true) {
            ...
            $valuesLabel = ArrayHelper::map($valuesLabel_obj, 'core_values_name', 'core_values_name');
            $today_valuesModel = new PilotFrontProductivityCheckin;
            $prev_values_currentday = PilotFrontProductivityCheckin::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])
                    ->andWhere(['!=', 'label', 'core_values_popup'])
    ...
    public function actionWelcome() {
        ...
        if (!empty($gamemodel)):
            if (((date("m-d-Y", $gamemodel->challenge_start_date)) <= $currentdate)) {
                return $this->redirect(Url::to(['/productivity/dashboard']));
            }
        endif;
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontProductivitySurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionFitnessSteps() {
        ...
        foreach ($gamedates_array as $key => $gamedate):
            ...
            $fetch_data = PilotFrontProductivityStepsApiData::find()->where(['user_id' => $user_id, 'date' => $convrtd_date])->one();
            if (!empty($fetch_data)):
    ...
    public function actionPostApiData() {
        ...
        if (isset($_REQUEST['uid']) && isset($_REQUEST['date']) && isset($_REQUEST['steps']) && isset($_REQUEST['calories']) && isset($_REQUEST['sleephr']) && isset($_REQUEST['distance'])):
            ...
            if (!empty($result_uid) && ($ruid > 0)):
                $getdata = PilotFrontProductivityStepsApiData::find()->where(['user_id' => $uid, 'date' => $date])->one();
                //Get the Challenge(Game) ID
                $game = PilotFrontUser::getGameID('productivity');
                //User Company ID
                ...
                $week_asper_date = PilotFrontUser::getGameWeekAsPerDate($game_obj, $dayset);
                if (empty($getdata)):
                    $api_model = new PilotFrontProductivityStepsApiData();
    ...
    public function actionTotalStepsModal() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $totalsteps_model = new PilotFrontProductivityStepsApiData();
        //My Total Steps for Complete Challenge
        $user_total_steps = PilotFrontProductivityStepsApiData::getUserStepsforchallenge($user_id);
        //Top five Users Total Steps for Complete Challenge
        $all_users_total_steps = PilotFrontProductivityStepsApiData::getAllUsersStepsforchallenge();
    ...
    public function actionReadCongrats() {
        ...
        date_default_timezone_set($timezone);
        $dayset = date('Y-m-d');
        $congratsModel = new PilotFrontProductivityStepsModal();
    ...
    public function actionTechSupport() {
        ...
        $model = new PilotFrontProductivityTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontProductivityTechSupport'];
            ...
            $newModel = new PilotFrontProductivityTechSupport();
    ...
    public function actionUserlist() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $challenge_id = Yii::$app->user->identity->challenge_id;
        $user_idlist = PilotFrontProductivityHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'game_id' => $challenge_id]);
...
//        }
    ...
    public function actionSeeAll() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $shareawins_all = PilotFrontProductivityNotifications::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])->orderBy(['created' => SORT_DESC]);
        $dataProvider = new ActiveDataProvider([
            'query' => $shareawins_all,
            'pagination' => [
                'pageSize' => 50,
    ...
    public function actionDailyEmail() {
        ...
        $daily_inspiration_obj = PilotDailyinspirationImage::find()->where(['category_id' => $daily_category_id, 'order_number' => $game_current_day_no])->one();
        $daily_image_path = Yii::getAlias('@back_end') . '/img/daily-inspiration-images/' . $daily_inspiration_obj->image_name;
        $filename = 'images/' . $daily_inspiration_obj->image_name;
        $daily_image_path = $daily_inspiration_obj->image_name;
        if (Yii::$app->request->post()):
            $messagecontent = '<html><body><p>' . Yii::$app->request->post()['PilotFrontProductivityEmail']['message'] . '</p><br><br><br><br><img src="' . $daily_image_path . '" alt="attachment" /></body></html>';
            $send_email = Yii::$app
                    ->mailer
                    ->compose('send-image', ['model' => Yii::$app->request->post()['PilotFrontProductivityEmail']['message'], 'daily_image_path' => $daily_image_path])
                    ->setFrom('support@injoyglobal.com')
                    ->setTo(Yii::$app->request->post()['PilotFrontProductivityEmail']['email_id'])
                    ->setSubject('Daily Inspiration Image')
                    ->send();
//            $message->send();
//            $to = Yii::$app->request->post()['PilotFrontLeadsEmail']['email_id'];
//            $from = 'support@injoyglobal.com';
...
//               $daily_email->email_id = Yii::$app->request->post()['PilotFrontLeadsEmail']['email_id'];
//                $daily_email->message = Yii::$app->request->post()['PilotFrontLeadsEmail']['message'];
...
//            }
            if ($send_email == 1):
                $daily_email->email_id = Yii::$app->request->post()['PilotFrontProductivityEmail']['email_id'];
                $daily_email->message = Yii::$app->request->post()['PilotFrontProductivityEmail']['message'];
                $daily_email->attachment = $daily_image_path;
                $daily_email->created = time();
                $daily_email->save(false);
     ...
     public function actionSaveRating() {
        ...
        $html = '';
        if (Yii::$app->request->post()):
            $model = PilotFrontProductivityRating::find()->where(['user_id' => $user_id, 'dayset' => $dayset, 'company_id' => $comp_id])->one();
            if (empty($model)):
                $model = new PilotFrontProductivityRating;
                $model->challenge_id = $game_obj->id;
                $model->game_id = $game;
                $model->company_id = $comp_id;
                $model->user_id = $user_id;
                $model->value = Yii::$app->request->post()['PilotFrontProductivityRating']['value'];
                $model->dayset = $dayset;
                $model->week_no = $week_no;
                $model->created = time();
                $model->updated = time();
