...
use frontend\models\PilotFrontTestHighfiveSearch;
...
use frontend\models\PilotFrontTestSurveyData;
use frontend\models\PilotFrontTestTechSupport;
...
use frontend\models\PilotFrontTestEmail;
...
class TestController extends Controller {
    ...
    public function actionDashboard() {
        ...
        if (isset($challenge_id)) {
            if ($challenge_id != '7' && $challenge_id != 0) {
...
//            $session->set('company_id', $company_obj->id);
        ...
        if (empty($game_obj)): //If there is No Ongoing Challenge but upcoming
            ...
            if (!empty($upcoming_challenge)):
                ...
                return $this->redirect(Url::to(['/learning/welcome']));
            else:
                return $this->redirect(Url::to(['/learning/thankyou']));
                return $this->goBack();
            endif;
            ...
            $dayset = date('Y-m-d');
            //Daily Inspiration
            $daily_entry = PilotFrontTestDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            $all_highfives = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit(20)->all();
            $prev_highfives_currentday = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment', 'dayset' => $dayset])->orderBy(['created' => SORT_ASC])->all();
            $hf_points = 10;
            $count_hf = 0;
            ...
            $highfiveModel = new PilotFrontTestHighfive;
            //Audio Challenge

            $audio_entry_weekly = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $audio_entry_currentday = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $audio_entry_first = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $audio_entry_sec = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();
            $audio_entry_third = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third', 'challenge_id' => $game_id])->one();

            // did you know corner

            $know_entry_weekly = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $know_entry_currentday = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $know_entry_first = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $know_entry_sec = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();

            // Question Corner

            $answer_entry_weekly = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'challenge_id' => $game_id])->all();
            $answer_entry_currentday = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
            $answer_entry_first = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first', 'challenge_id' => $game_id])->one();
            $answer_entry_sec = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second', 'challenge_id' => $game_id])->one();
            $answer_entry_third = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third', 'challenge_id' => $game_id])->one();
            ...
            if ($check_survey_active == 1):
                ...
                if (date('Y-m-d') >= $survey_live_date):
                    ...
                    $feedback_given = PilotFrontTestSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'Yes'])->all();
                    if (empty($feedback_given)):
                        //Check whelther User has skipped the Feedback Survey Form for Current Day
                        $feedback_skipped = PilotFrontTestSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No', 'dayset' => $dayset])->one();
            ...
            if (Yii::$app->request->post('PilotFrontTestSurveyData')):
                $skipped_entry = PilotFrontTestSurveyData::find()->where(['challenge_id' => $game_obj->id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
                if (!empty($skipped_entry)):
                    $skipped_entry->delete();
                endif;
                $surveyPostedData = Yii::$app->request->post('PilotFrontTestSurveyData');
                ...
                foreach ($survey_posted_questions as $key => $q_id):
                    $surveyModel = new PilotFrontTestSurveyData();
            ...
            return $this->render('dashboard', [
                        ...
                        'answer_entry_currentday' => $answer_entry_currentday,
                        'answer_entry_first' => $answer_entry_first,
                        'answer_entry_sec' => $answer_entry_sec,
                        'answer_entry_third' => $answer_entry_third,
            ]);
        endif;
    }
    ...
    public function actionHighFive() {
        ...
        $searchModel = new PilotFrontTestHighfiveSearch();
        $game = PilotFrontUser::getGameID('test');
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
    ...
    public function actionDailyModal() {
        ...
        $count_week_entr = 0;
        $points = 10;
        $week_entries = PilotFrontTestDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        ...
        //Makeup Days entry
//        $total_points_dailyInsp = PilotFrontTestDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id])
//                ->sum('points');
...
//        endif;
        ...
        $daily_model = new PilotFrontTestDailyinspiration;
        $daily_email = new PilotFrontTestEmail;
        $daily_entry = PilotFrontTestDailyinspiration::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
        if (Yii::$app->request->post()):
            $user_id = $_POST['user_id'];
            $dayset = $_POST['dayset'];
            $checkModel = PilotFrontTestDailyinspiration::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'dayset' => $dayset])->one();
            if (empty($checkModel)):
                $model = new PilotFrontTestDailyinspiration;
                $model->game_id = $game;
    ...
    public function actionHighfiveLike() {
        ...
        $checkModel = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])->one();
        if (empty($checkModel)):
            $model = new PilotFrontTestHighfive;
            $model->challenge_id = $game_id;
            $model->game_id = $game;
            $model->user_id = $user_id;
            $model->company_id = $comp_id;
            ...
            $linked_feature_user_model = PilotFrontTestHighfive::find()->where(['id' => $linked_feature_id])->one();
            ...
            if ($linked_feature_user->id != $user_id):
                $notif_model = new PilotFrontTestNotifications;
                $model->challenge_id = $game_id;
        ...
        $total_likes = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveLike', 'linked_feature_id' => $linked_feature_id])->all();
    ...
    public function actionTagList() {
        ...
        if ($_POST) {
            ...
            $query = "SELECT * FROM pilot_front_user where company_id = '$company_id' and challenge_id = '7' and firstname LIKE '$newstr'";
    ...
    public function actionHighfiveImageZoomModal() {
        ...
        $zoomImage = PilotFrontTestHighfive::find()->where(['id' => $img_id])->one();
        $html = $this->renderAjax('game_modals', [
            'zoomImage' => $zoomImage,
        ]);
        return $html;
    ...
    public function actionHighfiveUsercommentModal() {
        ...
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        $usercomment_model = new PilotFrontTestHighfive;
        $user_id = Yii::$app->request->get()['uid'];
        $comment_id = Yii::$app->request->get()['cid'];
        $feature_label = 'highfiveComment';
        $commentValue = PilotFrontTestHighfive::find()->where(['id' => $comment_id])->one();
        $commentImages = PilotFrontTestHighfive::find()->where(['feature_label' => 'highfiveCommentImage', 'linked_feature_id' => $comment_id])->all();
        $userComments = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $comment_id])
    ...
    public function actionSaveHighfiveUsercomment() {
        ...
        $checkModel = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'feature_serial' => $feature_serial, 'linked_feature_id' => $linked_feature_id])->one();
        $html = '';
        if (empty($checkModel)):
            $model = new PilotFrontTestHighfive;
            $model->challenge_id = $game_id;
        ...
        $allComments = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'linked_feature_id' => $linked_feature_id])
        ...
        //Total Comments
        if (!empty($team_id)):
            $total_comments = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        else:
            $total_comments = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveUserComment', 'linked_feature_id' => $linked_feature_id])->all();
        endif;
    ...
    public function actionGetNotifications() {
        ...
        $dayset = date('Y-m-d');
        $user_id = Yii::$app->user->identity->id;
        $user_notifs = PilotFrontTestNotifications::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->orderBy(['id' => SORT_DESC])->all();
        $notif_html = '';
    ...
    public function actionNotificationStatus() {
        $notif_id = $_GET['nid'];
        $notif_model = PilotFrontTestNotifications::find()->where(['id' => $notif_id])->one();
    ...
    public function actionWelcome() {
        ...
        if (!empty($gamemodel)):
            if (((date("m-d-Y", $gamemodel->challenge_start_date)) <= $currentdate)) {
                return $this->redirect(Url::to(['/test/dashboard']));
            }
        endif;
    ...
    public function actionAudioModal() {
        ...
        $audio_model = new PilotFrontTestWeeklychallenge;
        $audio_entry = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if ($_POST):
            $user_id = Yii::$app->user->identity->id;
            $tip_pos = $_POST['tip_pos'];
            $comment = $_POST['comment'];
            $dayset = $dayset;
            $checkModel = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
//            $total_points_leadership = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
//                    ->sum('points');
...
//            endif;
            //end here
            if (empty($checkModel)):
                $model = new PilotFrontTestWeeklychallenge;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionThankyou() {
        ...
        $gamemodel = PilotCreateGame::find()->where(['challenge_company_id' => $company_id, 'challenge_id' => $challenge_id, 'status' => 1])->one();
        if (!empty($gamemodel)):
            //echo date("m-d-Y", $gamemodel->challenge_end_date);
    ...
    public function actionSkipSurvey() {
        ...
        $check_already_skipped = PilotFrontTestSurveyData::find()->where(['challenge_id' => $challenge_id, 'user_id' => $user_id, 'survey_filled' => 'No'])->one();
    ...
    public function actionTechSupport() {
        ...
        //Active Challenge Object (Start Date & End Date) 
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $model = new PilotFrontTestTechSupport();

        if (Yii::$app->request->post()):
            $postedData = Yii::$app->request->post()['PilotFrontTestTechSupport'];
            ...
            $newModel = new PilotFrontTestTechSupport();
    ...
    public function actionSaveHighfiveImageComment() {
        ...
        $sum_week_points = 0;
        $week_entries_points = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no])
                ->sum('points');
        ...
        //Makeup Days entry           
//        $total_points_highfive = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
//                ->sum('points');
...
//        endif;
        ...
        /* $checkModel = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'feature_label' => $feature_label, 'dayset' => $dayset])->one();
          if (empty($checkModel)): */
        $model = new PilotFrontTestHighfive;
        $model->game_id = $game;
        $model->challenge_id = $game_id;
        ...
        if (isset($_SESSION['hf_cmnt_img'])):
            $image_model = new PilotFrontTestHighfive;
...
//              die;
    ...
    public function actionMoreHfComments() {
        ...
        $game_obj = PilotCreateGame::find()->where(['challenge_id' => $game, 'challenge_company_id' => $comp_id, 'status' => 1])->one();
        $game_id = $game_obj->id;
        //$subQuery = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveCommentImage', 'challenge_id' => $game_id])->select('linked_feature_id');
        $query = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC])->limit($count);
        $all_highfivecomment = $query->all();
        $fetched_comment = count($all_highfivecomment);
        $query1 = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])->orderBy(['created' => SORT_DESC, 'id' => SORT_DESC]);
    ...
    public function actionUserlist() {
        $search = $_POST['query'];
        $comp_id = Yii::$app->user->identity->company_id;
        $challenge_id = PilotFrontUser::getGameID('test');
        $user_idlist = PilotFrontTestHighfive::find()->select('user_id')->where(['company_id' => $comp_id, 'game_id' => $challenge_id]);
...
//        }
    ...
    public function actionKnowModal() {
        ...
        $dayset = date('Y-m-d');
        $tip_pos = Yii::$app->request->get()['tip'];
        $know_model = new PilotFrontTestKnowcorner;
        $know_entry = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        if ($_POST):
            $user_id = Yii::$app->user->identity->id;
            $tip_pos = $_POST['tip_pos'];
            $dayset = $dayset;
            $checkModel = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();

            //Makeup Days entry            
            $total_points_leadership = PilotFrontTestKnowcorner::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('points');
//            if (!empty($total_points_leadership) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
//                $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
//                $totalweek = $total_gameday / 7;
...
//            endif;
            //end here
            if (empty($checkModel)):
                $model = new PilotFrontTestKnowcorner;
                $model->challenge_id = $game_id;
                $model->game_id = $game;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
    ...
    public function actionAnswer1Modal() {
        $user_id = Yii::$app->user->identity->id;
        $game = PilotFrontUser::getGameID('test');
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 10;
        ...
        $dayset = date('Y-m-d');
        $tip_pos = Yii::$app->request->get()['tip'];
        $answer_entry = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => $tip_pos, 'challenge_id' => $game_id])->one();
        $answer1_model = array();
        $html = $this->renderAjax('new_modals', [
    ...
    public function actionAnswer2Modal() {
        $user_id = Yii::$app->user->identity->id;
        $game = PilotFrontUser::getGameID('test');
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 20;
    ...
    public function actionAnswer3Modal() {
        $user_id = Yii::$app->user->identity->id;
        $game = PilotFrontUser::getGameID('test');
        $comp_id = Yii::$app->user->identity->company_id;
        $points = 20;
    ...
    public function actionSaveAnswer() {
        ...
        $dayset = date('Y-m-d');
        //Makeup Days entry            
        $total_points_leadership = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                ->sum('points');
//        if (!empty($total_points_leadership) && $dayset >= date('Y-m-d', $game_obj->makeup_days)):
//            $total_gameday = PilotFrontUser::datediff($game_obj->challenge_start_date, $game_obj->challenge_end_date);
//            $totalweek = $total_gameday / 7;
//            $exactWeek = explode('.', $totalweek);
//            $total_game_points = $exactWeek[0] * 60;
//            if ($total_points_leadership >= $total_game_points):
//                $points = 0;
//            endif;
//        endif;
        $model = new PilotFrontTestQuestion;
        $model->game_id = $game;
        $model->challenge_id = $game_id;
        $model->user_id = $user_id;
        $model->company_id = $comp_id;
    ...
    public function actionGetUserPoints() {
        ...
        if (!empty($game_obj)):
            ...
            $check_entry = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->one();
            if (empty($check_entry)):
                $model = new PilotFrontTestTotalPoints;
                $model->game_id = $game;
                $model->challenge_id = $game_id;
                $model->user_id = $user_id;
                $model->company_id = $comp_id;
            ...
            $check_entry = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'user_id' => $user_id, 'company_id' => $comp_id])->one();
            $total_points = $check_entry->total_points;
        endif;
    ...
    public function actionToolbox() {
        ...
        //Leadership Corner
        $leadership_entry_weekly = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        $leadership_entry_currentday = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
        $leadership_entry_first = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
        $leadership_entry_sec = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
        $leadership_entry_third = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();
    ...
    public function actionKnowTheTeam() {
        ...
        $tip_no3 = $week_leadership . '-three';
        $week_pattern = $week_leadership . '%';
        $leadership_entry_weekly = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no])->all();
        $leadership_entry_currentday = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset])->one();
        $leadership_entry_first = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'first'])->one();
        $leadership_entry_sec = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'second'])->one();
        $leadership_entry_third = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'tip_pos' => 'third'])->one();
        if (empty($leadership_entry_weekly)):
            $tip = $tip_no1;
        endif;
...
// $answer_entry_second = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'user_id' => Yii::$app->user->identity->id, 'company_id' => $comp_id, 'week_no' => $week_no, 'dayset' => $dayset, 'challenge_id' => $game_id])->one();
        return $this->render('knowtheteam', ['tip' => $tip]);
    }
    ...
    public function actionLeaderboard() {
        ...
        foreach ($teams as $key => $team_id):
            ...
            if (!empty($team_id)):
                $points_obj = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $team_id])
                        ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
        ...
        endforeach;
        if (empty($teams[0])) {
            $points_obj = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->orderBy(['total_points' => SORT_DESC, 'updated' => SORT_DESC])
                    ->limit(10)
                    ->all();
        ...
        if (!empty($user_team_id)):
            $login_user_team_actions = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->sum('total_game_actions');
        else:
            $login_user_team_actions = PilotFrontTestTotalPoints::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])
                    ->sum('total_game_actions');
        endif;

        //Login User Team Total Service Values
        if (!empty($user_team_id)):
            $login_user_team_core_values = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                            ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])->count('id');
        else:
            $login_user_team_core_values = PilotFrontTestWeeklychallenge::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'challenge_id' => $game_id])->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_wins = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_wins = PilotFrontTestQuestion::find()->where(['game_id' => $game, 'company_id' => $comp_id])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
        ...
        if (!empty($user_team_id)):
            $login_user_team_high5s = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'team_id' => $user_team_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        else:
            $login_user_team_high5s = PilotFrontTestHighfive::find()->where(['game_id' => $game, 'company_id' => $comp_id, 'feature_label' => 'highfiveComment'])
                    ->andWhere(['between', 'created', $game_start_date_timestamp, $game_end_date_timestamp])
                    ->count('id');
        endif;
